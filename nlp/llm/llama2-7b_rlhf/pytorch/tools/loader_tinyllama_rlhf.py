...
def set_preprocess_state(args, model, model_tinyllama):
    '''Set embedding params.'''
    model.language_model.embedding.word_embeddings.weight.data.copy_(
        model_tinyllama["model.embed_tokens.weight"])
    # model.language_model.embedding.word_embeddings.weight.data.copy_(
    #     model_tinyllama["rwtranrsformer.embed_tokens.weight"])


def set_postprocess_state(args, model, model_tinyllama):
    '''Set output layer & norm params.'''
    # model.language_model.encoder.final_norm.weight.data.copy_(model_tinyllama["rwtranrsformer.norm.weight"])
    model.language_model.encoder.final_norm.weight.data.copy_(model_tinyllama["model.norm.weight"])
    # model.language_model.output_layer.weight.data.copy_(model_tinyllama["v_head.weight"])


def set_attn_state(args, layer, model_tinyllama, layer_name):
    '''Set self-attention params.'''

    # Get attention layer & state.
    attn = layer.self_attention
    layer_attn = layer_name + ".self_attn"
    ...
    attn.query_key_value.weight.data.copy_(torch.cat([ 
        model_tinyllama[layer_attn + ".q_proj.weight"].reshape((ng, dim*nh//ng, -1)),
        model_tinyllama[layer_attn + ".k_proj.weight"].reshape((ng, dim, -1)),
        model_tinyllama[layer_attn + ".v_proj.weight"].reshape((ng, dim, -1)),
    ], dim=1).reshape((-1, args.hidden_size)))
    attn.dense.weight.data.copy_(model_tinyllama[layer_attn + ".o_proj.weight"])


def set_mlp_state(args, layer, model_tinyllama, layer_name):
    '''Set MLP params.'''

    mlp = layer.mlp
    layer_mlp = layer_name + ".mlp"

    mlp.dense_h_to_4h.weight.data.copy_(torch.cat([
        model_tinyllama[layer_mlp + ".gate_proj.weight"],
        model_tinyllama[layer_mlp + ".up_proj.weight"],
    ], dim=0))
    mlp.dense_4h_to_h.weight.data.copy_(model_tinyllama[layer_mlp + ".down_proj.weight"])


def set_layer_state(args, model, model_tinyllama, layer_idx):
    '''Set transformer layer params.'''

    layer = model.language_model.encoder.layers[layer_idx]
    layer_name = "model.layers." + str(layer_idx)

    set_attn_state(args, layer, model_tinyllama, layer_name)
    set_mlp_state(args, layer, model_tinyllama, layer_name)
    layer.input_norm.weight.data.copy_(model_tinyllama[layer_name + ".input_layernorm.weight"])
    layer.post_attention_norm.weight.data.copy_(model_tinyllama[layer_name + ".post_attention_layernorm.weight"])


def load_checkpoint_to_model(args):
    '''Set model params.'''
    from pretrain_gpt_megatron import model_provider
    from transformers import LlamaForCausalLM

    # Load Huggingface model.
    model_tinyllama = torch.load(os.path.join(args.load, "pytorch_model.bin"))
    # hf_model = LlamaForCausalLM.from_pretrained(args.load, device_map="cpu")
    # print(f"1111111111111  {hf_model.lm_head.weight}")
    # Init Megatron model.
    if args.tinyllama:
        model = model_provider(True, True, rlhf_training=True).to(args.params_dtype)
    else:
        model = model_provider(True, True).to(args.params_dtype)

    # Set model state.
    set_preprocess_state(args, model, model_tinyllama)
    set_postprocess_state(args, model, model_tinyllama)

    for layer_idx in tqdm(range(args.num_layers), "set layer states"):
        set_layer_state(args, model, model_tinyllama, layer_idx)
...
def _load_checkpoint(queue, args):
    ...
    try:
        ...
        from megatron_ds.global_vars import set_args, set_global_variables
        from megatron_ds.model import module
