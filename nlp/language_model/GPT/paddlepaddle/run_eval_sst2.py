...
MODEL_CLASSES = {
    ...
    "gpt2": (GPTForSequenceClassification, GPTTokenizer),
        ...
        with open(args.eval_path, "r") as f:
            for line in f.readlines():
                if i>0:
                    ...
                    text = line[0]
                    # print(f'text: {text}')
                    tokens, _ = get_tokens(tokenizer, text)
                    labels = [int(line[1])]
...
def do_eval(args):
    ...
    if args.init_checkpoint_path is not None:
        ...
        # model = GPTForSequenceClassification(GPTModel(**model_class.pretrained_init_configuration[args.model_name]))
        model = GPTForSequenceClassification.from_pretrained("gpt2-medium-en", num_labels=2)
