...
def show_pred_gt(preds,
    ...
    pred_color = ocrcv.color_val('red')
    gt_color = ocrcv.color_val('blue')
    thickness = 1
    ...
    for gt in gts:
        cv2.polylines(
            ...
            thickness=thickness)
    if show:
        ocrcv.imshow(img, win_name, wait_time)
    if out_file is not None:
        ocrcv.imwrite(img, out_file)
...
def imshow_pred_boundary(img,
    ...
    boundary_color = ocrcv.color_val(boundary_color)
    text_color = ocrcv.color_val(text_color)
    font_scale = 0.5
...
def imshow_text_char_boundary(img,
    ...
    img = ocrcv.imread(img)
    char_color = [ocrcv.color_val('blue'), ocrcv.color_val('green')]
    text_color = ocrcv.color_val('red')
    text_inx = 0
...
def imshow_node(img,
    ...
    if show:
        ocrcv.imshow(vis_img, win_name, wait_time)
    if out_file is not None:
        ocrcv.imwrite(vis_img, out_file)
...
def det_recog_show_result(img, end2end_res, out_file=None):
    ...
    if out_file:
        ocrcv.imwrite(out_img, out_file)
...
def imshow_edge(img,
    ...
    if out_file is not None:
        ...
        ocrcv.dump(res_dic, f'{out_file}_res.pkl')
