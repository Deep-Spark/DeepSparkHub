...
class DecodeNode:
    ...
    def __init__(self, indexes=[1], scores=[0.9]):
        assert basic_utils.is_type_list(indexes, int)
        assert basic_utils.is_type_list(scores, float)
        assert basic_utils.equal_len(indexes, scores)
...
class ParallelSARDecoderWithBS(ParallelSARDecoder):
    ...
    def forward_test(self, feat, out_enc, img_metas):
        assert basic_utils.is_type_list(img_metas, dict)
