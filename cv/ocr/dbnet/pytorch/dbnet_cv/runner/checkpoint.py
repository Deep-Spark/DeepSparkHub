...
ENV_DBNET_CV_HOME = 'DBNET_CV_HOME'
ENV_XDG_CACHE_HOME = 'XDG_CACHE_HOME'
DEFAULT_CACHE_DIR = '~/.cache'


def _get_dbnet_cv_home():
    dbnet_cv_home = os.path.expanduser(
        os.getenv(
            ENV_DBNET_CV_HOME,
            os.path.join(
                os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), 'dbnet_cv')))

    mkdir_or_exist(dbnet_cv_home)
    return dbnet_cv_home
...
def get_torchvision_models():
    if digit_version(torchvision.__version__) < digit_version('0.13.0a0'):
        ...
        json_path = osp.join(dbnet_cv.__path__[0],
                             'model_zoo/torchvision_0.12.json')
        model_urls = dbnet_cv.load(json_path)
...
def get_external_models():
    dbnet_cv_home = _get_dbnet_cv_home()
    default_json_path = osp.join(dbnet_cv.__path__[0], 'model_zoo/open_mmlab.json')
    default_urls = load_file(default_json_path)
    assert isinstance(default_urls, dict)
    external_json_path = osp.join(dbnet_cv_home, 'open_mmlab.json')
...
def get_mmcls_models():
    mmcls_json_path = osp.join(dbnet_cv.__path__[0], 'model_zoo/mmcls.json')
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(dbnet_cv.__path__[0],
...
def load_from_openmmlab(
    ...
    if model_url.startswith(('http://', 'https://')):
        ...
        filename = osp.join(_get_dbnet_cv_home(), model_url)
        if not osp.isfile(filename):
            raise FileNotFoundError(f'{filename} can not be found.')
        checkpoint = torch.load(filename, map_location=map_location)
    return checkpoint
...
def save_checkpoint(model: torch.nn.Module,
    ...
    meta.update(dbnet_cv_version=dbnet_cv.__version__, time=time.asctime())
