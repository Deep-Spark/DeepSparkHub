...
from dbnet_det.core import BitmapMasks, PolygonMasks, find_inside_bboxes
from dbnet_det.core.evaluation.bbox_overlaps import bbox_overlaps
from dbnet_det.utils import log_img_scale
...
class Resize:
    ...
    def __init__(self,
        ...
        if img_scale is None:
            ...
            if isinstance(img_scale, list):
                ...
                self.img_scale = [img_scale]
            assert dbnet_cv.is_list_of(self.img_scale, tuple)
    ...
    def random_select(img_scales):
        ...
        assert dbnet_cv.is_list_of(img_scales, tuple)
    ...
    def random_sample(img_scales):
        ...
        assert dbnet_cv.is_list_of(img_scales, tuple) and len(img_scales) == 2
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                img, scale_factor = dbnet_cv.imrescale(
                    results[key],
                    results['scale'],
                    return_scale=True,
                ...
                # a real fix should be done in the dbnet_cv.imrescale in the future
                new_h, new_w = img.shape[:2]
                h, w = results[key].shape[:2]
                w_scale = new_w / w
                h_scale = new_h / h
            else:
                img, w_scale, h_scale = dbnet_cv.imresize(
                    results[key],
                    results['scale'],
                    return_scale=True,
                    interpolation=self.interpolation,
    ...
    def _resize_seg(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.keep_ratio:
                gt_seg = dbnet_cv.imrescale(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                gt_seg = dbnet_cv.imresize(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
...
class RandomFlip:
    ...
    def __init__(self, flip_ratio=None, direction='horizontal'):
        if isinstance(flip_ratio, list):
            assert dbnet_cv.is_list_of(flip_ratio, float)
        ...
        if isinstance(direction, str):
            ...
            assert dbnet_cv.is_list_of(direction, str)
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if self.size is not None:
                padded_img = dbnet_cv.impad(
                    results[key], shape=self.size, pad_val=pad_val)
            elif self.size_divisor is not None:
                padded_img = dbnet_cv.impad_to_multiple(
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        """
        for key in results.get('img_fields', ['img']):
            results[key] = dbnet_cv.imnormalize(results[key], self.mean, self.std,
                                            self.to_rgb)
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.scale_factor != 1:
                results[key] = dbnet_cv.imrescale(
                    results[key],
                    self.scale_factor,
                    interpolation='nearest',
                    backend=self.backend)
...
class MixUp:
    ...
    def _mixup_transform(self, results):
        ...
        retrieve_img = dbnet_cv.imresize(
        ...
        out_img = dbnet_cv.imresize(out_img, (int(out_img.shape[1] * jit_factor),
