...
def get_palette(palette, num_classes):
    ...
    if isinstance(palette, list):
        ...
        from dbnet_det.datasets import CocoDataset, CocoPanopticDataset
        dataset_palette = CocoDataset.PALETTE
        if len(dataset_palette) < num_classes:
            dataset_palette = CocoPanopticDataset.PALETTE
    elif palette == 'citys':
        from dbnet_det.datasets import CityscapesDataset
        dataset_palette = CityscapesDataset.PALETTE
    elif palette == 'voc':
        from dbnet_det.datasets import VOCDataset
        dataset_palette = VOCDataset.PALETTE
    elif dbnet_cv.is_str(palette):
        dataset_palette = [dbnet_cv.color_val(palette)[::-1]] * num_classes
    else:
