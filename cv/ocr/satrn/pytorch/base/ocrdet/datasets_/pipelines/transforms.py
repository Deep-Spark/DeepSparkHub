...
from ocrdet.core import PolygonMasks
from ocrdet.core.evaluation.bbox_overlaps import bbox_overlaps
...
class Resize:
    ...
    def __init__(self,
        ...
        if img_scale is None:
            ...
            if isinstance(img_scale, list):
                ...
                self.img_scale = [img_scale]
            assert ocrcv.is_list_of(self.img_scale, tuple)
    ...
    def random_select(img_scales):
        ...
        assert ocrcv.is_list_of(img_scales, tuple)
    ...
    def random_sample(img_scales):
        ...
        assert ocrcv.is_list_of(img_scales, tuple) and len(img_scales) == 2
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                img, scale_factor = ocrcv.imrescale(
                    results[key],
                    results['scale'],
                    return_scale=True,
                    backend=self.backend)
                # the w_scale and h_scale has minor difference
                # a real fix should be done in the ocrcv.imrescale in the future
                new_h, new_w = img.shape[:2]
                h, w = results[key].shape[:2]
                w_scale = new_w / w
                h_scale = new_h / h
            else:
                img, w_scale, h_scale = ocrcv.imresize(
                    results[key],
                    results['scale'],
                    return_scale=True,
                    backend=self.backend)
    ...
    def _resize_seg(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.keep_ratio:
                gt_seg = ocrcv.imrescale(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                gt_seg = ocrcv.imresize(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
...
class RandomFlip:
    ...
    def __init__(self, flip_ratio=None, direction='horizontal'):
        if isinstance(flip_ratio, list):
            assert ocrcv.is_list_of(flip_ratio, float)
        ...
        if isinstance(direction, str):
            ...
            assert ocrcv.is_list_of(direction, str)
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if self.size is not None:
                padded_img = ocrcv.impad(
                    results[key], shape=self.size, pad_val=pad_val)
            elif self.size_divisor is not None:
                padded_img = ocrcv.impad_to_multiple(
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        """
        for key in results.get('img_fields', ['img']):
            results[key] = ocrcv.imnormalize(results[key], self.mean, self.std,
                                            self.to_rgb)
...
class MixUp:
    ...
    def _mixup_transform(self, results):
        ...
        out_img = ocrcv.imresize(out_img, (int(out_img.shape[1] * jit_factor),
