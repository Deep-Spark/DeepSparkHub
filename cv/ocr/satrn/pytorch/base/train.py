...
import ocrcv
import torch
from ocrcv import Config, DictAction
from ocrcv.runner import get_dist_info, init_dist, set_random_seed

from apis import init_random_seed, train_detector
from datasets_ import build_dataset
from models import build_detector
from basic_utils import (collect_env, get_root_logger, is_2dlist,
...
def replace_root_isneed(cfg_dict, cur_key, pre_dataroot, last_dataroot):
    if cur_key in cfg_dict.keys():
        cfg_dict[cur_key] = cfg_dict[cur_key].replace(pre_dataroot, last_dataroot, 1)
...
def main():
    ...
    pre_dataroot = args.fake_data_root
    if args.data_root is not None:
        if 'datasets' in cfg.data.train.keys():
            for dataset in cfg.data.train.datasets:
                replace_root_isneed(dataset, 'ann_file', pre_dataroot, args.data_root)
                replace_root_isneed(dataset, 'img_prefix', pre_dataroot, args.data_root)

        if 'datasets' in cfg.data.val.keys():
            for dataset in cfg.data.val.datasets:
                replace_root_isneed(dataset, 'ann_file', pre_dataroot, args.data_root)
                replace_root_isneed(dataset, 'img_prefix', pre_dataroot, args.data_root)

        if 'datasets' in cfg.data.test.keys():
            for dataset in cfg.data.test.datasets:
                replace_root_isneed(dataset, 'ann_file', pre_dataroot, args.data_root)
                replace_root_isneed(dataset, 'img_prefix', pre_dataroot, args.data_root)
    ...
    # create work_dir
    ocrcv.mkdir_or_exist(osp.abspath(cfg.work_dir))
    # dump config
