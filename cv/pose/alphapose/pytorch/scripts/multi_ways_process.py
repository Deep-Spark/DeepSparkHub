...
multi process:  process data
single process: model forward(batch_size = the number of process data)
"""
...
def load_det_data(orig_im, inp_path, q):
    ...
    q.put((img, orig_im, dim, im_name))


def load_det_data_(inp_path_, inp_path, q):

    dataset = LoadStreams(inp_path_)
    for j, img in dataset:
        orig_im = img[0] 
        break
...
def pose_post(pose_model_out, q):
...
def run():
    ...
    # stage1 multiprocess load data(detection)
    stage1 = []
    stage1_q = torch.multiprocessing.Queue()
    for i in range(n):
        img = cv2.imread(inp_path_list[i])
        p1 = Process(target=load_det_data, args=(img, '', stage1_q)) 
        stage1.append(p1) 
    
    for p in stage1:
        p.start()

    det_inps0 = [stage1_q.get() for p in stage1]
    ...
    # stage3 multiprocess post process(detection)
    stage3 = []
    stage3_q = torch.multiprocessing.Queue()
    for i in range(n):
        p3 = Process(target=det_post, args=((det_out[i:i+1],) + det_inps0[i][1:], stage3_q))
        stage3.append(p3)
            
    for p3 in stage3:
        p3.start()
    det_res = [stage3_q.get() for p3 in stage3]
    
    # stage4 multiprocess load data(pose)
    stage4 = []
    stage4_q = torch.multiprocessing.Queue()
    
    for i in range(n):
        p4 = Process(target=load_pose_data, args=(det_res[i], stage4_q))
        stage4.append(p4)

    for p4 in stage4:
        p4.start()
    pose_inps0 = [stage4_q.get() for p4 in stage4]
    ...
    # stage6 multiprocess post process(alphapose)
    stage6 = []
    stage6_q = torch.multiprocessing.Queue()

    for i in range(n):
        p6 = Process(target=pose_post, args=((pose_model_out[i:i+1], ) + pose_inps0[i][1:] , stage6_q))
        stage6.append(p6)
    
    for p6 in stage6:
        p6.start()
    pose_res = [stage6_q.get() for p6 in stage6]
