...
parser.add_argument('--imgsdir', 
                    type=str, 
                    default="./examples/batch_imgs/singleway", 
...
def batched_nms(prediction, conf_thres=0.25, iou_thres=0.45, max_det=100):
    ...
    return output, idxs_batch


def batched_det_post_to_pose_data(prediction, idxs_batch, orig_img, im_dim, im_name):
    pose_inps = []
    pose_infor = []
    
    prediction[:, :4] = scale_coords(args.inp_size, prediction[:, :4], (1280, 720)).round()
    dets = prediction.cpu()
    
    for i, (y, z) in enumerate(zip(orig_img, im_name)):    
        det = dets[idxs_batch == i]
        boxes = det[:, :4]
        ...
        pose_infor.append((y, z, boxes[flag][0:1], scores[flag][0:1], ids[flag][0:1], cropped_boxes[flag][0:1]))
        pose_inps.append(inps)
    
    pose_inps = torch.cat(pose_inps, dim=0)
    
    return pose_inps, pose_infor
...
def detection(data, det_model):
    ...
    prediction, idxs_batch = batched_nms(prediction)
    pose_inps, pose_infor = batched_det_post_to_pose_data(prediction, idxs_batch, orig_img, im_dim, im_name)
...
def pose(pose_inps, pose_infor, pose_model):
    pose_out = []
    with torch.no_grad():
    ...
    if args.FP16:
        hm = hm.float()
    for hm_data, infor in zip(hm, pose_infor):
        res = pose_post(hm_data.unsqueeze(0), infor)
        pose_out.append(res)
    return pose_out
...
def all_pipeline_time(det_model, pose_model):
    ...
    for i in range(256):
        ...
        pose_inps, pose_infor = detection(data, det_model)
        pose_out = pose(pose_inps, pose_infor, pose_model)
        e = time.time()
        time_once = e - s
...
def test_pose(det_model, pose_model):
    rootdir = './examples/batch_imgs/singleway'
    imgnames = [x for x in os.listdir(rootdir) if x.endswith('png') or x.endswith('jpg')]
    imgpaths = [os.path.join(rootdir, imgname) for imgname in imgnames]    
