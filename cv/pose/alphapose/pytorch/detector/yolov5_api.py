...
class YOLODetector(BaseDetector):
    ...
    def load_model(self):
        ...
        self.model = DetectMultiBackend(weights='./yolov5/yolov5s.pt', device=device, dnn=False)
    ...
    def images_detection(self, imgs, orig_dim_list):
        ...
        with torch.no_grad():
            ...
            prediction = self.model(imgs, augment=False, visualize=False) 
            ...
            # prediction = non_max_suppression(prediction, conf_thres, iou_thres, classes, agnostic_nms, max_det=max_det)
            prediction = non_max_suppression(prediction, 0.25, 0.45, None, False, max_det=10)
            prediction_cat = []
            for tmp in prediction:
                num_objs, _ = tmp.shape
                if num_objs < 10:
                    pad = torch.zeros((10-num_objs, 6)).to(tmp.device)
                    tmp = torch.cat([tmp, pad], dim=0)    
                prediction_cat.append(tmp)
            prediction = torch.stack(prediction_cat)
