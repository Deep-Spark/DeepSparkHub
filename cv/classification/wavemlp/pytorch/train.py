...
parser.add_argument('--apex-opt-level', type=int, default=1,
                    help='Use NVIDIA Apex Opt-level')
...
def main():
    ...
    def backward_hook(module, grad_input, grad_output):
        ...
        #print("input:")
        #print([x.shape if x is not None else 0 for x in grad_input])
        #print("ouput:")
        #print([x.shape if x is not None else 0 for x in grad_output])

    def forward_hook(module, fea_input, fea_output):
        #print("==="*20)
        print('Inside ' + module.__class__.__name__ + ' forward')
        #print("input: ")
        #print([x.shape if x is not None else 0 for x in fea_input])
        #print("ouput:")
        #print([x.shape if x is not None else 0 for x in fea_output])
    ...
    loss_scaler = None
    if use_amp == 'apex':
        model, optimizer = amp.initialize(model, optimizer, opt_level="O{}".format(args.apex_opt_level))
    ...
    if args.local_rank == 0:
        ...
        saver = CheckpointSaver(
            ...
            checkpoint_dir=output_dir, recovery_dir=output_dir, decreasing=decreasing, max_history=100)
