...
PYTHONARG='-m torch.distributed.launch --nproc_per_node auto --use_env --master_port 10005'
# For debugging
if [ ! -z "${DEBUG}" ];then
...
python3  $PYTHONARG ${ROOT_DIR}/run_train.py  \
	--model resnet101 --dali --dali-cpu  --data-path $DATA_PATH  \
	--opt fused_sgd --batch-size 300   \
	--amp --nhwc --padding-channel "$@"
