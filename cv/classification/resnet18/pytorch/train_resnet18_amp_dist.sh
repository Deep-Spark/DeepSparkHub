...
PYTHONARG='-m torch.distributed.launch --nproc_per_node auto --use_env --master_port 10008'
# For debugging
if [ ! -z "${DEBUG}" ];then
...
python3  $PYTHONARG ${ROOT_DIR}/run_train.py \
	--model resnet18 --dali --dali-cpu  --data-path $DATA_PATH \
	--opt fused_sgd --batch-size 1024 \
	--amp --nhwc "$@"
