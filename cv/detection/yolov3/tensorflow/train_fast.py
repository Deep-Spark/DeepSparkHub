...
#   File name   : train.py
#   Author      : Liwei Dai
#   Created date: 2021/4/30 15:30
#   Description : Fast training: only run second stage training for limited steps.
#
#================================================================
...
class YoloTrain(object):
    def __init__(self):
        ...
        self.first_stage_epochs  = 2
        self.second_stage_epochs = 0
        self.second_stage_steps  = 100
        ...
        with tf.name_scope('learn_rate'):
            ...
            train_steps = tf.constant( (self.first_stage_epochs + self.second_stage_epochs)* self.steps_per_period
                                        + self.second_stage_steps, dtype=tf.float32, name='train_steps')
    ...
    def train(self):
        ...
        for epoch in range(1, 1+self.first_stage_epochs+self.second_stage_epochs):
            ...
            if epoch > 1 + self.first_stage_epochs:
            ...
            print("\tSaving to %s.%s..." % (ckpt_file, epoch))
            self.saver.save(self.sess, ckpt_file, global_step=epoch)

        # Second stage training, unfreeze all weights
        train_op = self.train_op_with_all_variables
        pbar = tqdm(range(self.second_stage_steps))
        train_epoch_loss, test_epoch_loss = [], []

        for idx in pbar:
            train_data = next(self.trainset)
            _, summary, train_step_loss, global_step_val = self.sess.run(
                [train_op, self.write_op, self.loss, self.global_step],feed_dict={
                                            self.input_data:   train_data[0],
                                            self.label_sbbox:  train_data[1],
        ...
        log_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
        print("=> Epoch: %2d Time: %s Train loss: %.2f..."
                %(epoch+1, log_time, train_epoch_loss))

        # evaluate on testset in the end        
        pbar_test = tqdm(self.testset)
        for test_data in pbar_test:
            test_step_loss = self.sess.run( self.loss, feed_dict={
                                            self.input_data:   test_data[0],
        ...
        print("=> Epoch: %2d Time: %s Train loss: %.2f Test loss: %.2f ..."
                        %(epoch, log_time, train_epoch_loss, test_epoch_loss))
        # Save checkpoints
        ckpt_file = "./checkpoint/yolov3_train_fast.ckpt" 
        print("\tSaving to %s..." % ckpt_file)
