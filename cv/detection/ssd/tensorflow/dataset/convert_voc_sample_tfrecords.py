...
tf.app.flags.DEFINE_string('dataset_directory', '/media/rs/7A0EE8880EE83EAF/Detections/PASCAL/VOC',
                           'All datas directory')
tf.app.flags.DEFINE_string('train_splits', 'VOC2012',
                           'Comma-separated list of the training data sub-directory')
tf.app.flags.DEFINE_string('validation_splits', 'VOC2012',
...
def _process_dataset(name, directory, all_splits, num_shards):
  ...
  for split in all_splits:
    image_names_file = os.path.join(directory, split, 'ImageSets/Main', name + '.txt')
    with open(image_names_file) as f:
      image_names = f.readlines()
    images = [_in.strip() + '.jpg' for _in in image_names]
    print(f"split {split} | name {name} | num images {len(images)}")
