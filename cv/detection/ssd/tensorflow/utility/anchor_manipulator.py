...
# Fix bug by adjust the position of arguments on TensorFlow 1.15
def tf_name_scope_wrapper(tf_name_scope):
    def _real(*args, **kwargs):
        total_args = len(args) + len(kwargs)
        if total_args == len(args) == 2:
            if isinstance(args[1], str):
                return tf_name_scope(*args, **kwargs)
            else:
                return tf_name_scope(args[0], args[0], args[1])
        return tf_name_scope(*args, **kwargs)
    return _real

tf.name_scope = tf_name_scope_wrapper(tf.name_scope)
