...
def get_backbone(backbone, training, **kwargs):
    forward_fn = _VGG_BACKBONES[backbone](training, **kwargs)
    return forward_fn


def get_vgg16(training, **kwargs):
    _backbone = VGG16Backbone(**kwargs)
    return partial(_backbone.forward, training=training)


def get_resnet18(training, **kwargs):
    _backbone = resnet18(training, **kwargs)
    return _backbone


def get_resnet34(training, **kwargs):
    _backbone = resnet34(training, **kwargs)
    return _backbone


def get_resnet50(training, **kwargs):
    _backbone = resnet50(training, **kwargs)
    return _backbone


def get_resnet101(training, **kwargs):
    _backbone = resnet101(training, **kwargs)
    return _backbone
...
def ssd_conv_block(
    filters, strides, name,
    data_format, kernel_initializer,
    padding='same', reuse=None):
...
class SSDBackbone(object):
    def __init__(self, backbone, training, **kwargs):
        self.backbone = get_backbone(backbone, training, **kwargs)
        self.training = training
        self.data_format = kwargs.get('data_format', 'channels_first')

        # SSD layers
        with tf.variable_scope('additional_layers') as scope:
            # down_5
            self._conv8_block = ssd_conv_block(256, 2, 'conv8',
                data_format=self.data_format,
                kernel_initializer=tf.glorot_uniform_initializer())
            # down_6
            self._conv9_block = ssd_conv_block(128, 2, 'conv9',
                data_format=self.data_format,
                kernel_initializer=tf.glorot_uniform_initializer())
            self._conv10_block = ssd_conv_block(128, 1, 'conv10', padding='valid',
                data_format=self.data_format,
                kernel_initializer=tf.glorot_uniform_initializer())
            self._conv11_block = ssd_conv_block(128, 1, 'conv11', padding='valid',
                data_format=self.data_format,
                kernel_initializer=tf.glorot_uniform_initializer())

    def forward(self, inputs):
        feature_layers = self.backbone(inputs)
        if len(feature_layers) > 2:
            feature_layers = [feature_layers[-3], feature_layers[-2]]
        else:
            feature_layers = [feature_layers[-2], feature_layers[-1]]
        inputs = feature_layers[-1]

        # forward ssd layers
        for layer in self._conv8_block:
            inputs = forward_module(layer, inputs, training=self.training)
        # conv8
