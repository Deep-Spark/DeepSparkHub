...
def do_train(
    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
        ...
        fps = len(images.image_sizes) / (batch_time + data_time) * get_world_size()
        meters.update(time=batch_time, data=data_time, fps=fps)

        eta_seconds = meters.time.global_avg * (max_iter - iteration)
        eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))

        if iteration % arguments["log_freq"] == 0 or iteration == max_iter:
            logger.info(
