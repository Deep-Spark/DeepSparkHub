...
class ENet(SegBaseModel):
    ...
    def __init__(self, cfg, **kwargs):
        super(ENet, self).__init__(cfg, need_backbone=False)
    ...
    def forward(self, x):
        ...
        # stage 5
        if x.shape != b1_shape:
            x = F.upsample(x, b1_shape[2:], mode="bilinear")
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, inter_channels, out_channels, dilation=1, asymmetric=False,
        ...
        if downsampling:
            self.maxpool = nn.MaxPool2d(3, 2, return_indices=True, padding=1)
        ...
        if downsampling:
            self.conv2 = nn.Sequential(
                nn.Conv2d(inter_channels, inter_channels, 3, stride=2, padding=1, bias=False),
                norm_layer(inter_channels),
                nn.PReLU()
            )
        else:
