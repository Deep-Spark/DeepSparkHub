...
class ResNetV1(nn.Module):
    ...
    def __init__(self, block, layers, cfg=None, num_classes=1000, deep_stem=False,
        ...
        self.layer4 = self._make_layer(block, int(512 * scale), layers[3], stride=strides[1], dilation=dilations[1],
                                       norm_layer=norm_layer, multi_grid=cfg.MODEL.MULTI_GRID,
                                       multi_dilation=cfg.MODEL.MULTI_DILATION)
...
def resnet18(cfg=None, norm_layer=nn.BatchNorm2d):
    num_block = [2, 2, 2, 2]
    return ResNetV1(BasicBlockV1b, num_block, cfg=cfg, norm_layer=norm_layer)


def resnet34(cfg=None, norm_layer=nn.BatchNorm2d):
    num_block = [3, 4, 6, 3]
    return ResNetV1(BasicBlockV1b, num_block, cfg=cfg, norm_layer=norm_layer)


def resnet50(cfg=None, norm_layer=nn.BatchNorm2d):
    num_block = [3, 4, 6, 3]
    return ResNetV1(BottleneckV1b, num_block, cfg=cfg, norm_layer=norm_layer)


def resnet101(cfg=None, norm_layer=nn.BatchNorm2d):
    num_block = [3, 4, 23, 3]
    return ResNetV1(BottleneckV1b, num_block, cfg=cfg, norm_layer=norm_layer)


def resnet152(cfg=None, norm_layer=nn.BatchNorm2d):
    num_block = [3, 8, 36, 3]
    return ResNetV1(BottleneckV1b, num_block, cfg=cfg, norm_layer=norm_layer)


def resnet50c(cfg=None, norm_layer=nn.BatchNorm2d):
    num_block = [3, 4, 6, 3]
    return ResNetV1(BottleneckV1b, num_block, cfg=cfg, norm_layer=norm_layer, deep_stem=True)


def resnet101c(cfg=None, norm_layer=nn.BatchNorm2d):
    num_block = [3, 4, 23, 3]
    return ResNetV1(BottleneckV1b, num_block, cfg=cfg, norm_layer=norm_layer, deep_stem=True)


def resnet152c(cfg=None, norm_layer=nn.BatchNorm2d):
    num_block = [3, 8, 36, 3]
    return ResNetV1(BottleneckV1b, num_block, cfg=cfg, norm_layer=norm_layer, deep_stem=True)
