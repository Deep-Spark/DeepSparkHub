...
class DABNet(SegBaseModel):
    ...
    """
    def __init__(self, cfg, block_1=3, block_2=6):
        super().__init__(cfg, need_backbone=False)
        inp = 3 + int(padding_channel())
        self.init_conv = nn.Sequential(
            Conv(inp, 32, 3, 2, padding=1, bn_acti=True),
        ...
        self.bn_prelu_1 = BNPReLU(32 + inp)

        # DAB Block 1
        self.downsample_1 = DownSamplingBlock(32 + inp, 64)
        self.DAB_Block_1 = nn.Sequential()
        for i in range(0, block_1):
            self.DAB_Block_1.add_module("DAB_Module_1_" + str(i), DABModule(64, d=2))
        self.bn_prelu_2 = BNPReLU(128 + inp)

        # DAB Block 2
        dilation_block_2 = [4, 4, 8, 8, 16, 16]
        self.downsample_2 = DownSamplingBlock(128 + inp, 128)
        self.DAB_Block_2 = nn.Sequential()
        for i in range(0, block_2):
            self.DAB_Block_2.add_module("DAB_Module_2_" + str(i),
                                        DABModule(128, d=dilation_block_2[i]))
        self.bn_prelu_3 = BNPReLU(256 + inp)

        self.classifier = nn.Sequential(Conv(259 + int(padding_channel()), self.nclass, 1, 1, padding=0))
