...
class EESPNet(nn.Module):
    def __init__(self, cfg=None, num_classes=1000, scale=1, reinf=True, norm_layer=nn.BatchNorm2d):
        super(EESPNet, self).__init__()
        inp_reinf = 4 if padding_channel() else 3
        reps = [0, 3, 7, 3]
        r_lim = [13, 11, 9, 7, 5]
        K = [4] * len(r_lim)
...
def eespnet(cfg=None, norm_layer=nn.BatchNorm2d):
    return EESPNet(cfg=cfg, norm_layer=norm_layer)
