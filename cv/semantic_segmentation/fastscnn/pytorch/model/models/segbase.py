...
class SegBaseModel(nn.Module):
    ...
    """
    def __init__(self, cfg, need_backbone=True):
        super(SegBaseModel, self).__init__()
        self.cfg = cfg
        self.nclass = cfg.MODEL.NUM_CLASS
    ...
    def get_backbone(self):
        self.backbone = self.cfg.MODEL.BACKBONE.lower()
        self.encoder = get_segmentation_backbone(self.backbone, self.cfg, self.norm_layer)
    ...
    def evaluate(self, image):
        ...
        scales = self.cfg.TEST.SCALES
        flip = self.cfg.TEST.FLIP
        crop_size = _to_tuple(self.cfg.TEST.CROP_SIZE) if self.cfg.TEST.CROP_SIZE else None
