...
at::Tensor layer_norm_gradient(
  ...
  if (mean.has_value()) {
    cuda_layer_norm_gradient(&dout,&mean.value(),&invvar,&input,n1,n2,
        normalized_shape,NULL,NULL,epsilon,
        &grad_input,NULL,NULL,memory_efficient);
  } else {
    cuda_layer_norm_gradient(&dout,NULL,&invvar,&input,n1,n2,
        normalized_shape,NULL,NULL,epsilon,
        &grad_input,NULL,NULL,memory_efficient);
  }
  return grad_input;
...
std::vector<at::Tensor> rms_norm_pre_norm_residual_forward(
    at::Tensor input,
    at::Tensor residual,
    #ifdef VERSION_GE_1_1
    at::IntArrayRef normalized_shape,
  ...
  at::Tensor invvar = at::empty({n1}, input.options().dtype(input.scalar_type() == at::ScalarType::Half || input.scalar_type() == at::ScalarType::BFloat16 ? at::ScalarType::Float : input.scalar_type()));
   ...
   cuda_rms_norm_residual(&output,&sum,&invvar, &input, &residual, n1, n2,
      normalized_shape, &gamma,epsilon);
  return {output,invvar,sum};
}
...
std::vector<at::Tensor> rms_norm_pre_norm_residual_backward(
    at::Tensor dout,
    at::Tensor dres,
    at::Tensor invvar,
    at::Tensor input,
  ...
  at::Tensor grad_input = at::empty_like(input);
  at::Tensor grad_gamma = at::empty_like(gamma);
  cuda_rms_norm_residual_gradient(&dout,&dres,&invvar,&input,n1,n2,
      normalized_shape,&gamma,epsilon,
      &grad_input,&grad_gamma,memory_efficient);
  return {grad_input, grad_gamma};
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("rms_pre_norm_residual_forward", &rms_norm_pre_norm_residual_forward, "RMSNorm forward with pre norm residual");
  m.def("rms_pre_norm_residual_backward", &rms_norm_pre_norm_residual_backward, "RMSNorm backward with pre norm residual");
}
