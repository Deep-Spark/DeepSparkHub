...
static void _report_aio_statistics(const char* tag,
                                   const std::vector<std::chrono::duration<float>>& latencies)
    __attribute__((unused));

static void _report_aio_statistics(const char* tag,
                                   const std::vector<std::chrono::duration<float>>& latencies)
{
    std::vector<float> lat_usec;
...
static void _get_aio_latencies(std::vector<std::chrono::duration<float>>& raw_latencies,
...
static void _do_io_submit_singles(const int64_t n_iocbs,
                                  ...
                                  std::vector<std::chrono::duration<float>>& submit_times)
{
...
static int _do_io_complete(const int64_t min_completes,
                           ...
                           std::unique_ptr<aio_context>& aio_ctxt,
                           std::vector<std::chrono::duration<float>>& reap_times)
{
                                 ...
                                 deepspeed_aio_perf_t* perf)
{
    ...
    const auto num_io_blocks = static_cast<long long int>(
        ceil(static_cast<float>(xfer_ctxt->_num_bytes) / aio_ctxt->_block_size));
    ...
    for (int64_t iocb_index = 0; iocb_index < num_io_blocks; iocb_index += aio_ctxt->_queue_depth) {
        ...
        const auto n_iocbs =
            min(static_cast<long long int>(aio_ctxt->_queue_depth), (num_io_blocks - iocb_index));
