...
class PipelineModule(nn.Module):
    ...
    Args:
        ...
        custom_partition (list, optional): custom model layers in PP stage.
        ...
        custom_recompute_layers_per_stage (list, optional): custom recompute layers in PP stage (for megatron-deepspeed).
    """
    ...
    def __init__(self,
                 ...
                 dynamic_shape=False,
                 custom_recompute_layers_per_stage=None):
        ...
        self._partition_layers(method=partition_method, custom_partition=custom_partition)
        ...
        self.custom_activation_checkpoint = None
        if custom_recompute_layers_per_stage is not None:
            self.custom_activation_checkpoint = custom_recompute_layers_per_stage
    ...
    def forward(self, forward_input):
        ...
        num_layers = len(self.forward_funcs)
        if self.custom_activation_checkpoint is not None:
            recompute_layers = self.custom_activation_checkpoint[self._grid.get_pipe_parallel_rank()]
            ## 获取pp stage 第一层transformer的序号
            for i in range(num_layers-1):
                f = self.forward_funcs[i:i+1]
                if self._is_checkpointable(f):
                    break
            if recompute_layers != 0:
                activation_checkpoint_interval = (num_layers - i) // recompute_layers
                ## 当重计算层数大于transformer 层数，则全部重计算
        ...
        if (activation_checkpoint_interval == 0) or (self.activation_checkpoint_interval == 0):
            ...
            for start_idx, is_checkpointable_result in \
                ...
                if self.custom_activation_checkpoint is not None:
                    if checkpoint_num_layers < recompute_layers - 1:
                        end_idx = min(start_idx + activation_checkpoint_interval, num_layers)
                    else:
                        end_idx = num_layers
                else:
                    end_idx = min(start_idx + activation_checkpoint_interval, num_layers)
                ...
                ## 计算完最后一组时，退出
                if end_idx == num_layers:
                    break
        return x

    def _partition_layers(self, method='uniform', custom_partition=None):
        ...
        ## custom partition of layers to pp stage
        elif method == 'custom':
            num_layers = len(self._layer_specs)
            if not custom_partition:
                raise ValueError(f"argument of custom-partition should not be None, it should be number layers of every PP_stage. Or choose other partition-methon like: uniform")
            if isinstance(custom_partition, list):
                custom_part = custom_partition
            if len(custom_part) != num_stages:
                raise ValueError(f"lenth of custom-partition should be equal to PP_stages. lenth of custom={len(custom_part)}  num_stages={num_stages}")
            if sum(custom_part) != num_layers:
                raise ValueError(f"sum of custom-partition layers should be equal to total model layers. sum(custom_part)={sum(custom_part)}  num_layers={num_layers}")
            self.parts = [0]*(num_stages + 1)
            for i in range(num_stages):
                self.parts[i+1] = self.parts[i] + custom_part[i]
        elif method == 'profile':
