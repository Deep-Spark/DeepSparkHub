...
class DeepSpeedZeRoOffload(object):
    ...
    def __init__(
        ...
        self.use_ds_opt = True if os.getenv("OPT_MODULES", None) else False
        print_rank_0(f"Stage_3 param_numel_persistence_threshold {self.param_numel_persistence_threshold}", force=self.use_ds_opt)
        print_rank_0(f"Stage_3 max_reuse_distance: {self._max_reuse_distance_in_numel}", force=self.use_ds_opt)
        print_rank_0(f"Stage_3 max_live_parameters: {self._max_available_parameters_in_numel}", force=self.use_ds_opt)
    ...
    def partition_all_parameters(self):
        ...
        for param in iter_params(self.module, recurse=True):
            if not self.use_ds_opt:
                if param.ds_status != ZeroParamStatus.NOT_AVAILABLE:
                    raise RuntimeError(f"{param.ds_summary()} expected to be released")
            else:
                if param.ds_status == ZeroParamStatus.HOLD_COMPLETE:
                    continue
                elif param.ds_status != ZeroParamStatus.NOT_AVAILABLE:
                    raise RuntimeError(f"{param.ds_summary()} expected to be released")
    ...
    def _register_hooks_recursively(self, module, count=[0]):
        ...
        def _pre_forward_module_hook(module, *args):
            self.pre_sub_module_forward_function(module, *args)
    ...
    @torch.no_grad()
    def pre_sub_module_forward_function(self, sub_module, *args):
        ...
        param_coordinator.fetch_sub_module(sub_module, forward=True, args=args)
