...
from deepspeed.ops.op_builder import FusedLayernormBuilder
...
class FusedLayerNormAffineFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input, grad_weight, grad_bias = fused_layer_norm_cuda.backward_affine(
            grad_output.contiguous(), mean, invvar, input_, ctx.normalized_shape, weight_, bias_, ctx.eps, ctx.memory_efficient
        )
...
class FusedRMSNormAffineFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input, grad_weight = fused_layer_norm_cuda.rms_backward_affine(
           grad_output.contiguous(), invvar, input_, ctx.normalized_shape, weight_, ctx.eps, ctx.memory_efficient
        )
...
class FusedLayerNormFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = fused_layer_norm_cuda.backward(
            grad_output.contiguous(), mean, invvar, input_, ctx.normalized_shape, ctx.eps, False
        )
...
class FusedRMSNormFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = fused_layer_norm_cuda.rms_backward(
            grad_output.contiguous(), invvar, input_, ctx.normalized_shape, ctx.eps, False
        )
        return grad_input, None, None

class FusedRMSNormResidualFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, weight, residual, normalized_shape, eps, memory_efficient=True):
        ctx.normalized_shape = normalized_shape
        ctx.eps = eps
        ctx.memory_efficient = memory_efficient
        input_ = input.contiguous()
        weight_ = weight.contiguous()
        residual_ = residual.contiguous()
        output, invvar, sum_res = fused_layer_norm_cuda.rms_pre_norm_residual_forward(input_, residual_, ctx.normalized_shape, weight_, ctx.eps)
        if memory_efficient:
            ctx.save_for_backward(weight_, output, invvar)
        else:
            ctx.save_for_backward(weight_, sum_res, invvar)
        return output, sum_res

    @staticmethod
    def backward(ctx, grad_output, grad_res):
        weight_, input_or_output, invvar = ctx.saved_tensors
        grad_input = grad_weight = None
        grad_input, grad_weight = fused_layer_norm_cuda.rms_pre_norm_residual_backward(
            grad_output.contiguous(), grad_res.contiguous(), invvar, input_or_output, ctx.normalized_shape, weight_, ctx.eps, ctx.memory_efficient
        )
        return grad_input, grad_weight, grad_input, None, None, None
