...
def split_half_float_double(tensors):
    ...
    dtypes = [
        "torch.{}.HalfTensor".format(device_type), "torch.{}.FloatTensor".format(device_type), "torch.{}.BFloat16Tensor".format(device_type)
    ]
    buckets = []
...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def reduce_independent_p_g_buckets_and_remove_grads(self, param, i):
        ...
        if self.contiguous_gradients:
            if param.numel() > self.reduce_bucket_size:
                ...
                new_grad_tensor.copy_(grad_reduc.reshape(-1))
    ...
    def set_norm_for_param_grad(self, param):
        ...
        self.norm_for_param_grads[param_id] = accumulated_grad.data.float().norm(2)
        ...
        if norm_type == inf:
            ...
            for g, p in zip(gradients, params):
                ...
                if is_model_parallel_parameter(p) or (self.model_parallel_rank == 0):
                    ...
                    all_norms.append(
                        torch.norm(g.data.float().detach(), norm_type).to(get_accelerator().current_device_name()))
