...
class PartitionedParameterCoordinator:
    ...
    def __init__(
        ...
        self.opt_modules = os.getenv("OPT_MODULES", None)
        if self.opt_modules:
            self.limit_module_id = os.getenv("LIMIT_MODULE_ID", "0,100").split(',')
            import math
            self.use_norm_pad = 32
            self.limit_shape =  math.ceil(int(os.getenv("LIMIT_SHAPE", 500)) / self.use_norm_pad) * self.use_norm_pad

            self.opt_modules = self.opt_modules.upper().split(',')
            self.record_opt_modules = list()
            self.device = torch.cuda.current_device()
            self.force_release_false = torch.tensor(0, device=self.device, dtype=torch.int32)
            self.force_release_true  = torch.tensor(1, device=self.device, dtype=torch.int32)
            self.stop_current_step_opt = False
            self.cur_shape = -1
    ...
    @instrument_w_nvtx
    @torch.no_grad()
    def fetch_sub_module(self, current_submodule: Module, forward: bool, args=None) -> None:
        ...
        if self.opt_modules and forward and current_submodule._get_name().upper() == 'EMBEDDING':
            self.cur_shape = args[0][0].shape[-1] if args is not None else 0
            force_tensor = self.force_release_true if self.cur_shape > self.limit_shape else self.force_release_false
            dist.all_reduce(force_tensor, group=dist.get_world_group())
            force_release = True if (force_tensor > 0) else False

            if force_release:
                self.stop_current_step_opt = True
                if len(self.record_opt_modules) > 0:
                    # 需要把opt参数都释放掉
                    for opt_module in self.record_opt_modules:
                        if hasattr(opt_module, "HOLD_COMPLETE"):
                            opt_module.__delattr__("HOLD_COMPLETE")
                        params = set(p for p in iter_params(opt_module, recurse=z3_leaf_module(opt_module)))
                        for p in params:
                            if p.ds_status == ZeroParamStatus.HOLD_COMPLETE:
                                p.ds_status = ZeroParamStatus.AVAILABLE
                            p.ds_active_sub_modules.discard(opt_module.id)
                            self.__release_param(p)
                    self.record_opt_modules.clear()
        ...
        # opt参数已经常驻
        already_complete_cached = False
        if self.opt_modules and not self.stop_current_step_opt and (current_submodule._get_name().upper() in self.opt_modules) and hasattr(current_submodule, "HOLD_COMPLETE"):
            for p in params_to_fetch:
                if p.ds_status == ZeroParamStatus.HOLD_COMPLETE:
                    already_complete_cached = True
                    break
        run_fetch = not already_complete_cached if self.opt_modules else True

        if run_fetch and fetch_numel > 0:
    ...
    @instrument_w_nvtx
    @torch.no_grad()
    def release_sub_module(self, submodule: Module, forward=True) -> None:
        """release the parameters of a sub module, assuming they meet conditions to
        be released."""
        if self.opt_modules:
            is_opt_submodule_name = (submodule._get_name().upper() in self.opt_modules)
            # opt参数常驻
            limit_flag = (self.__step_id > int(self.limit_module_id[0]) and self.__step_id < int(self.limit_module_id[1]))
            if not self.stop_current_step_opt and is_opt_submodule_name and (limit_flag or hasattr(submodule, "HOLD_COMPLETE")):
                if not hasattr(submodule, "HOLD_COMPLETE"):
                    submodule.__setattr__("HOLD_COMPLETE", True)
                    self.record_opt_modules.append(submodule)
                params = set(p for p in iter_params(submodule, recurse=z3_leaf_module(submodule)))
                for p in params:
                    if p.ds_status != ZeroParamStatus.HOLD_COMPLETE:
                        assert p.ds_status == ZeroParamStatus.AVAILABLE, p.ds_summary()
                        p.ds_status = ZeroParamStatus.HOLD_COMPLETE
                return
    ...
    def release_and_reset_all(self, module: Module) -> None:
        ...
        for param in iter_params(module, recurse=True):
            if self.opt_modules and param.ds_status == ZeroParamStatus.HOLD_COMPLETE:
