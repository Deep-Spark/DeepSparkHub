...
#ifdef __ILUVATAR__
    g._data.coalesced.mask = __internal::lanemask_eq();
#else
...
class thread_block : public thread_group
{
	...
	// Invalid, immediately fail
        if (tilesz == 0 || (tilesz > warpSize) || !pow2_tilesz) {
	    __internal::abort();
            return (thread_block());
	}
	...
	mask = (uint64_t) (-1) >> (warpSize - masklength);
	mask <<= (__internal::laneid() & ~(tilesz - 1));
	thread_group tile = thread_group(__internal::CoalescedTile);
        tile._data.coalesced.mask = mask;
        tile._data.coalesced.size = __popcll(mask);
        tile._data.coalesced.metaGroupSize= (__internal::cta::size() + tilesz -1) / tilesz;
        tile._data.coalesced.metaGroupSize= __internal::cta::thread_rank() / tilesz;
#else
	// Invalid, immediately fail
...
class coalesced_group : public thread_group
{
	...
	for (int bit_idx = 0; bit_idx < warpSize; bit_idx++) {
            uint64_t lane_bit = _data.coalesced.mask & (1 << bit_idx);
            if (lane_bit) {
...
#ifdef __ILUVATAR__
     _CG_QUALIFIER unsigned int thread_rank() const {
        return (__popcll(_data.coalesced.mask & __internal::lanemask_lt()));
    }
#else
    _CG_QUALIFIER unsigned int thread_rank() const {
...
# ifdef _CG_HAS_BFLOAT16_SUPPORT
    COALESCED_SHFL_FUNCTION(__nv_bfloat16);
    COALESCED_SHFL_UP_FUNCTION(__nv_bfloat16);
    COALESCED_SHFL_DOWN_FUNCTION(__nv_bfloat16);

    COALESCED_SHFL_FUNCTION(__nv_bfloat162);
    COALESCED_SHFL_UP_FUNCTION(__nv_bfloat162);
    COALESCED_SHFL_DOWN_FUNCTION(__nv_bfloat162);
# endif
...
#undef COALESCED_SHFL_DOWN_FUNCTION
    ...
    _CG_QUALIFIER int any(int predicate) const {
        return (__ballot_sync((unsigned int)_data.coalesced.mask, predicate) != 0);
    }
    _CG_QUALIFIER int all(int predicate) const {
        return (__ballot_sync((unsigned int)_data.coalesced.mask, predicate) == _data.coalesced.mask);
    }
    _CG_QUALIFIER unsigned int ballot(int predicate) const {
        if (size() == 32) {
            return (__ballot_sync(0xFFFFFFFF, predicate));
        }
        unsigned int lane_ballot = __ballot_sync((unsigned int)_data.coalesced.mask, predicate);
...
#ifdef _CG_HAS_BFLOAT16_SUPPORT
    _CG_QUALIFIER __nv_bfloat16 shfl(__nv_bfloat16 var, int srcRank) const {
        return (__shfl_sync(build_mask(), var, srcRank, numThreads));
    }
    _CG_QUALIFIER __nv_bfloat16 shfl_down(__nv_bfloat16 var, unsigned int delta) const {
        return (__shfl_down_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER __nv_bfloat16 shfl_up(__nv_bfloat16 var, unsigned int delta) const {
        return (__shfl_up_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER __nv_bfloat16 shfl_xor(__nv_bfloat16 var, unsigned int laneMask) const {
        return (__shfl_xor_sync(build_mask(), var, laneMask, numThreads));
    }
    _CG_QUALIFIER __nv_bfloat162 shfl(__nv_bfloat162 var, int srcRank) const {
        return (__shfl_sync(build_mask(), var, srcRank, numThreads));
    }
    _CG_QUALIFIER __nv_bfloat162 shfl_down(__nv_bfloat162 var, unsigned int delta) const {
        return (__shfl_down_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER __nv_bfloat162 shfl_up(__nv_bfloat162 var, unsigned int delta) const {
        return (__shfl_up_sync(build_mask(), var, delta, numThreads));
    }
    _CG_QUALIFIER __nv_bfloat162 shfl_xor(__nv_bfloat162 var, unsigned int laneMask) const {
        return (__shfl_xor_sync(build_mask(), var, laneMask, numThreads));
    }
...
template <> class thread_block_tile<64> : public __thread_block_tile_base<64> { };
...
namespace __internal {
    ...
    template <unsigned int Size>
    struct tiled_partition_impl<Size, thread_block_tile<64> > : public thread_block_tile<Size> {
        _CG_QUALIFIER tiled_partition_impl(thread_block_tile<64> const&) : thread_block_tile<Size>() {}
    };
    template <unsigned int Size>
