...
is_corex = hasattr(torch, "corex") and torch.corex
if torch_available and get_accelerator().device_name() == 'cuda' and not is_rocm_pytorch and not is_corex:
...
if command_exists('git') and not is_env_set('DEEPSPEED_LOCAL_VERSION_IDENTIFIER'):
...
if is_env_set('DEEPSPEED_LOCAL_VERSION_IDENTIFIER'):
    local_version_identifier = os.environ.get("DEEPSPEED_LOCAL_VERSION_IDENTIFIER")
    # Build string env specified, probably building for distribution.
    with open('build.txt', 'w') as fd:
        fd.write(os.environ['DEEPSPEED_LOCAL_VERSION_IDENTIFIER'])
    version_str += "+" + local_version_identifier
