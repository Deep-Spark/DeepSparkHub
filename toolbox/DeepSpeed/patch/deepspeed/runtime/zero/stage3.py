...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def _constant_buffered_norm2(self, input, buffer_size=250000000):
        ...
        for part in input.view(-1).split(buffer_size):
            if norm is None:
                norm = part.data.float().norm(2)**2.0
            else:
                norm += part.data.float().norm(2)**2.0
        return norm**0.5
    ...
    def set_norm_for_param_grad_in_gpu(self, param):
        param_id = self.get_param_id(param)
        #self.norm_for_param_grads[param_id] = param.grad.data.float().norm(2)
    ...
    def get_grad_norm_direct(self, gradients, params, norm_type=2):
        ...
        if norm_type == inf:
            ...
            for g, p in zip(gradients, params):
                if is_model_parallel_parameter(p) or (self.model_parallel_rank == 0):
                    grad_norms.append(g.to(get_accelerator().device_name(), non_blocking=True).float().norm(2))
