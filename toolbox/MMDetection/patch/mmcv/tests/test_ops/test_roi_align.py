...
def _test_roialign_gradcheck(device, dtype):
    ...
    for case in inputs:
        ...
        if torch.__version__ == 'parrots':
            ...
            gradcheck(froipool, (x, rois), eps=1e-3, atol=1e-2)
...
def _test_roialign_allclose(device, dtype):
    ...
    for case, output in zip(inputs, outputs):
        ...
        assert np.allclose(
            output.data.type(torch.float).cpu().numpy(), np_output, rtol=1e-2, atol=1e-2)
        assert np.allclose(
            x.grad.data.type(torch.float).cpu().numpy(), np_grad, rtol=1e-2, atol=1e-2)
