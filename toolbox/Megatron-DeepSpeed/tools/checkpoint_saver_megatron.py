...
def save_huggingface_llama(args, model, model_args, global_layer_num):
    '''Set model params.'''
    ...
    for name_param_m in model.named_parameters():
        ...
        if len(name_param_m[0].split(".")) > 4:
            model_layer_num = str(int(layer_num) + global_layer_num)
        ...
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.post_attention_norm.weight":
            hf2mg_map[f"model.layers.{model_layer_num}.post_attention_layernorm.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.input_norm.weight":
            hf2mg_map[f"model.layers.{model_layer_num}.input_layernorm.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.post_attention_norm.weight":
            hf2mg_map[f"model.layers.{model_layer_num}.post_attention_layernorm.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.self_attention.query_key_value.weight":
            ...
            hf2mg_map[f"model.layers.{model_layer_num}.self_attn.q_proj.weight"] = qw
            hf2mg_map[f"model.layers.{model_layer_num}.self_attn.k_proj.weight"] = kw
            hf2mg_map[f"model.layers.{model_layer_num}.self_attn.v_proj.weight"] = vw
            continue
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.self_attention.query_key_value.bias":
            ...
            hf2mg_map[f"model.layers.{model_layer_num}.self_attn.q_proj.bias"] = qw
            hf2mg_map[f"model.layers.{model_layer_num}.self_attn.k_proj.bias"] = kw
            hf2mg_map[f"model.layers.{model_layer_num}.self_attn.v_proj.bias"] = vw
            continue
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.self_attention.dense.bias":
            hf2mg_map[f"model.layers.{model_layer_num}.self_attn.dense.bias"] = name_param_m[1]
            continue
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.self_attention.dense.weight":
            hf2mg_map[f"model.layers.{model_layer_num}.self_attn.o_proj.weight"] = name_param_m[1]
            continue
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.mlp.dense_h_to_4h.weight":
            ...
            hf2mg_map[f"model.layers.{model_layer_num}.mlp.gate_proj.weight"] = name_param_m[1][:proj_read_h_half, ...]
            hf2mg_map[f"model.layers.{model_layer_num}.mlp.up_proj.weight"] = name_param_m[1][proj_read_h_half:, ...]
            continue
        if name_param_m[0] == f"language_model.encoder.layers.{layer_num}.mlp.dense_4h_to_h.weight":
            hf2mg_map[f"model.layers.{model_layer_num}.mlp.down_proj.weight"] = name_param_m[1]
            continue
...
def save_checkpoint(queue, args):
    ...
    try:
        ...
        from megatron_ds.global_vars import set_global_variables, get_args
        from megatron_ds.core.enums import ModelType
        from megatron_ds.tokenizer.tokenizer import _vocab_size_with_padding
    ...
    def get_models(count, dtype, pre_process, post_process):
        if args.tinyllama:
            models = [model_provider(pre_process, post_process, rlhf_training=True).to(dtype) for _ in range(count)]
        else:
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        if post_process:
            ...
            if md.output_layer:
                ...
                if not args.tinyllama:
                    ...
                    for tp_rank in range(args.target_tensor_parallel_size):
                        models[tp_rank].language_model.output_layer.weight.data.copy_(output_layer_weight)
        ...
        for tp_rank in range(args.target_tensor_parallel_size):
            ...
            if args.save_model_type == 'megatron':
                ...
                if pp_rank > 0:
                    global_layer_num += current_layer_num
                save_huggingface_llama(args, models[tp_rank], md, global_layer_num)
                current_layer_num = len(models[tp_rank - 1].language_model.encoder.layers)
    print("Done!")
