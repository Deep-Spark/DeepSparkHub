...
def forward_step(
    ...
    old_log_probs=None, advantages=None, action_mask=None,
    old_values=None, returns=None,
    model_prefix=None
):
    ...
    with context_manager:
        # actor model train
        if old_log_probs is not None and advantages is not None and action_mask is not None:
            output_tensor, loss_func = forward_step_func(prompts, model,
                                            old_log_probs, advantages, action_mask)
        # critic model train
        elif old_values is not None and returns is not None and action_mask is not None:
            output_tensor, loss_func = forward_step_func(prompts, model, old_values=old_values, 
                                              returns=returns, action_mask=action_mask)
        else:
            output_tensor, loss_func = forward_step_func(prompts, model, model_prefix)
...
def forward_backward_pipelining_without_interleaving(
    *,
    ...
    for i in range(num_warmup_microbatches):
        ...
        # 取 micro batch 数据
        old_log_probs_1, advantages_1, action_mask_1, old_values_1, returns_1 = None, None, None, None, None
        if old_log_probs is not None:
            old_log_probs_1 = old_log_probs[data_index:data_index+micro_batch_size]
        if advantages is not None:
            advantages_1 = advantages[data_index:data_index+micro_batch_size]
        if action_mask is not None:
            action_mask_1 = action_mask[data_index:data_index+micro_batch_size]
        if old_values is not None:
            old_values_1 = old_values[data_index:data_index+micro_batch_size]
        if returns is not None:
            returns_1 = returns[data_index:data_index+micro_batch_size]

        output_tensor = forward_step(
            forward_step_func,
            prompts[data_index:data_index+micro_batch_size],
            model,
            num_microbatches,
            input_tensor,
            forward_data_store,
            ...
            collect_non_loss_data,
            checkpoint_activations_microbatch,
            old_log_probs=old_log_probs_1,
            advantages=advantages_1,
            action_mask=action_mask_1,
            old_values=old_values_1,
            returns=returns_1,
            model_prefix=model_prefix,
        )
        data_index += micro_batch_size
