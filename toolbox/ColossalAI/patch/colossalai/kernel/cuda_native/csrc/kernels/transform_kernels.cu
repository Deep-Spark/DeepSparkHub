...
gridDim.y = seq_len
blockDim.x = std::min(hidden_dim, MAX_THREADS)
...
void launch_transform_0213<float>(float *output, const float *input,
  ...
  dim3 block_dim(std::min(hidden_dim, MAX_THREADS));
...
gridDim.y = dim_1
gridDim.z = dim_2
blockDim.x = std::min(dim_3 * dim_4, MAX_THREADS)
...
void launch_bias_add_transform_20314<float>(float *output, const float *input,
  ...
  dim3 block_dim(std::min(dim_3 * dim_4, MAX_THREADS));
