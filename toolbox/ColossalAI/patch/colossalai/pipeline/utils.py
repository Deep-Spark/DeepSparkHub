...
def partition_balanced(weights, pipeline_parallel_size, num_chunks, model_prefix=None):
    ...
    # 针对 gpt2 模型 pipeline last stage 进行优化，避免最后一个 stage 只有 head 层，而导致 attention_mask 梯度消失
    if model_prefix == "gpt2" and len(intervals) > 1 and intervals[-1][1] - intervals[-1][0] == 2:
        last = intervals.pop()
        pre_last = intervals.pop()
        intervals.append((pre_last[0], pre_last[1]+1))
        intervals.append((last[0]+1, last[1]))
