...
from colossalai.performance import PerformanceEvaluator
from colossalai.utils import get_batch_on_this_tp_rank
...
class SFTTrainer(SLTrainer):
    ...
    def _train(self, epoch: int):
        ...
        for step in tqdm(range(len(self.train_dataloader)), desc="Step", disable=not self.coordinator.is_master()):
            self.performance_evaluator.on_step_start(step)

            batch = get_batch_on_this_tp_rank(self.args, data_iter, self.plugin.tp_group, self.plugin.pp_group)

            if hasattr(self.plugin, "pp_size") and self.plugin.pp_size > 1:
            ...
            if dist.get_rank() == dist.get_world_size() - 1:
                print(f"Step {step} loss: {global_loss.item()}")
            ...
            if (self.num_train_step + 1) % self.accumulation_steps == 0:
                ...
                if hasattr(self.accumulative_meter, "loss"):
                    if self.writer:
            ...
            self.performance_evaluator.on_step_end()
        ...
        self.performance_evaluator.on_fit_end()
