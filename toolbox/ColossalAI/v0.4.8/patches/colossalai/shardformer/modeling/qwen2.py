...
class Qwen2PipelineForwards:
    ...
    def qwen2_model_forward(
        ...
        num_ckpt_layers = 0
        if shard_config.gradient_checkpoint_config is not None and self.training:
...
def get_qwen2_decoder_layer_forward():
    '''
        由于 post_attention_layernorm 使用了 ixformer FusedRMSNormRes 融合算子,
        输入输出格式需要改动, 因此针对 transformers Qwen2DecoderLayer 进行迁移
    '''
    def forward(
        self,
        hidden_states: torch.Tensor,
