...
class OneForwardOneBackwardSchedule(PipelineSchedule):
    def __init__(
        ...
        assert (microbatch_size is not None), "Either microbatch_size should be provided"
    ...
    def load_batch(self, batch: Dict, device: Optional[torch.device] = None) -> None:
    ...
    def forward_backward_step(
        ...
        if self.forward_only:
            result = self.run_forward_only(model, batch, criterion, return_loss, return_outputs)
        else:
            result = self.run_forward_backward(model, batch, criterion, optimizer, return_loss, return_outputs)
