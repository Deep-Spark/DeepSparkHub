...
class ThroughputMetric(Metric):
    ...
    def update(self, num_samples, time) -> None:
        ...
        if self.cur_steps >= self.ignored_steps:
            ...
            self.accumulated_tflops += self._tflop_per_step
    ...
    def get_last_step_info(self) -> str:
        ...
        if self._tflop_per_step > 0:
            ...
            return f"{sample_per_sec}, tflops={tflops}"
        else:
            return f"{sample_per_sec}"
    ...
    def get_average_tflops(self):
        return self.accumulated_tflops / (self.accumulated_used_time + 1e-12)
...
class ThroughputHook(MetricHook):
    ...
    def after_hook_is_attached(self, trainer):
        ...
        if self._is_stage_to_compute:
            ...
            # register the metric
            trainer.states['metrics']['train']['sample_per_sec'] = self.metric
            trainer.states['metrics']['test']['sample_per_sec'] = self.metric
