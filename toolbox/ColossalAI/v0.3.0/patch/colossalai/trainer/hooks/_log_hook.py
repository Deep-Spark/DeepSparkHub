...
class LogMetricByEpochHook(LogByEpochHook):
    ...
    def _get_str(self, trainer, mode):
        ...
        for metric_name, metric_calculator in trainer.states['metrics'][mode].items():
            if metric_name == "sample_per_sec":
                msg.append(f'average sample_per_sec = {_format_number(metric_calculator.get_accumulated_value())}')
                msg.append(f'average tflops = {_format_number(metric_calculator.get_average_tflops())}')
            else:
