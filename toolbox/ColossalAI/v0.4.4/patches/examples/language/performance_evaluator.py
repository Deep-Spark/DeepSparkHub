...
class PerformanceEvaluator:
    ...
    def on_fit_end(self) -> None:
        ...
        avg_tokens_per_gpu = avg_throughput * self.seq_len / self.coordinator.world_size / 2
        ...
        self.coordinator.print_on_master(
            f"num_samples: {self.num_samples}, dp_world_size: {self.dp_world_size}, flop_megatron: {self.flop_megatron}, flop: {self.flop}, avg_duration: {avg_duration:.2f}, "
            f"avg_throughput: {avg_throughput:.2f}"
        )
        self.coordinator.print_on_master(
            f"Tokens per GPU per Second:{avg_tokens_per_gpu:.2f}, Throughput: {avg_throughput:.2f} samples/sec, TFLOPS per GPU by Megatron: {avg_tflops_per_gpu_megatron:.2f}, TFLOPS per GPU: {avg_tflops_per_gpu:.2f}"
        )
