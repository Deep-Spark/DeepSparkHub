...
class LinearWithAsyncCommunication(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        return grad_input, grad_weight, grad_bias, None, None, None, None
class LinearWithFusedGradAccu(torch.autograd.Function):
    """
    Linear layer execution with FusedGradient.
    """

    @staticmethod
    def forward(ctx, input_, weight, bias = None):
        ctx.save_for_backward(input_, weight, bias)
        ctx.use_bias = bias is not None
    ...
    def backward(ctx, grad_output):
        ...
        if _grad_accum_fusion_available and weight.grad is not None:
            ...
            if grad.dtype == torch.float32:
                fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp32(total_input, grad_output, grad)
                grad_weight = None
            # wgrad_gemm_accum_fp16 接口同时支持fp16和bf16
            elif grad.dtype == torch.float16 or grad.dtype == torch.bfloat16:
                fused_weight_gradient_mlp_cuda.wgrad_gemm_accum_fp16(total_input, grad_output, grad)
                grad_weight = None
            else:
                grad_weight = grad_output.t().matmul(total_input)
