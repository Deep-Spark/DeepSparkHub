...
try:
    from ixformer.train import FusedRMSNorm
    class FusedRMSNormWithHook(FusedRMSNorm):
        def __init__(self, normalized_shape, eps=0.00001, elementwise_affine=True, gradient_accumulation_fusion = True):
            super().__init__(normalized_shape, eps, elementwise_affine, gradient_accumulation_fusion = gradient_accumulation_fusion)
    ...
    warnings.warn("Please install ixformer  to use the fused rmsnorm kernel")
...
from ixformer.train import FusedRMSNormRes
class Colo_FusedRMSNorm(nn.Module):
    def __init__(self) -> None:
        raise NotImplementedError(
            "Colo_FusedRMSNorm is not implemented as a physical class. "
            "It is meant to be used only with the from_native_module interface to Convert a native LlamaRMSNorm(from transformers) module to RMSNormResidualOrNot module provided above."
        )
    ...
    def from_native_module(module: nn.Module, *args, **kwargs) -> nn.Module:
        ...
        normalized_shape = getattr(module, "normalized_shape", module.weight.size())
        eps = module.variance_epsilon if hasattr(module, "variance_epsilon") else module.eps

        FusedRMSNorm = FusedRMSNormRes(normalized_shape, 
                                         eps=eps,
                                         gradient_accumulation_fusion = True)
        FusedRMSNorm.weight = module.weight
        # FusedRMSNorm.weight.grad = torch.zeros_like(FusedRMSNorm.weight)

        # print(f"rank:{torch.distributed.get_rank()}, weight:{FusedRMSNorm.weight.size()}, grad:{FusedRMSNorm.weight.grad.size()}")
        return FusedRMSNorm
