...
class EPOptimizeMixtralSparseMoeBlock(ParallelModule):
    def __init__(self, *args, **kwargs):
        raise RuntimeError(
            f"Please use `from_native_module` to create an instance of {self.__class__.__name__}"
        )
    ...
    def setup_process_groups(
        ...
        if self.num_experts % self.ep_size != 0:
            raise ValueError(
                "The number of experts must be divisible by the number of expert parallel groups."
            )

        self.num_experts_per_ep = self.num_experts // self.ep_size
        self.expert_start_idx = self.ep_rank * self.num_experts_per_ep
        held_experts = self.experts[
            self.expert_start_idx : self.expert_start_idx + self.num_experts_per_ep
        ]
    ...
    def from_native_module(
        ...
        **kwargs,
        ...
        # TODO: better init
        LazyInitContext.materialize(module)
        module.__class__ = EPOptimizeMixtralSparseMoeBlock
        fp8_communication = kwargs.get("fp8_communication", False)
        module.setup_process_groups(tp_group, moe_dp_group, ep_group, fp8_communication)
        return module
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        dist.all_to_all_single(
            output_split_sizes, input_split_sizes, group=self.ep_group
        )

        with torch.no_grad():
            activate_experts = output_split_sizes[: self.num_experts_per_ep].clone()
            for i in range(1, self.ep_size):
                activate_experts += output_split_sizes[
                    i * self.num_experts_per_ep : (i + 1) * self.num_experts_per_ep
                ]
            activate_experts = (activate_experts > 0).float()
        ...
        input_split_list = (
            input_split_sizes.view(self.ep_size, self.num_experts_per_ep)
            .sum(dim=-1)
            .tolist()
        )
        output_split_list = (
            output_split_sizes.view(self.ep_size, self.num_experts_per_ep)
            .sum(dim=-1)
            .tolist()
        )
        ...
        if output_states.size(0) > 0:
            if self.num_experts_per_ep == 1:
                # no need to split
                expert = self.experts[self.expert_start_idx]
                output_states = DPGradScalerIn.apply(
                    output_states, self.moe_dp_size, activate_experts[0]
                )
                output_states = expert.act_fn(expert.w1(output_states)) * expert.w3(
                    output_states
                )
                output_states = expert.w2(output_states)
                output_states = DPGradScalerOut.apply(
                    output_states, self.moe_dp_size, activate_experts[0]
                )
            else:
                output_states_splits = output_states.split(output_split_sizes.tolist())
                tmp_output_states_list = []
                for i in range(self.num_experts_per_ep):
                    split_states = output_states_splits[i]
                    if split_states.size(0) == 0:
                        continue
                    expert = self.experts[
                        self.expert_start_idx + i % self.num_experts_per_ep
                    ]
                    split_states = DPGradScalerIn.apply(
                        split_states,
                        self.moe_dp_size,
                        activate_experts[i % self.num_experts_per_ep],
                    )
                    split_states = expert.act_fn(expert.w1(split_states)) * expert.w3(
                        split_states
                    )
                    split_states = expert.w2(split_states)
                    split_states = DPGradScalerOut.apply(
                    ...
                    tmp_output_states_list.append(split_states)
                # 重复 len(output_split_list) 次
                output_states_list_1 = []
                for i in range(len(output_split_list)):
                    output_states_list_1.extend(tmp_output_states_list)

                output_states = torch.cat(output_states_list_1)
