...
class LlamaPipelineForwards:
    ...
    def llama_model_forward(
        ...
        elif shard_config.use_colo_llamaflashatten: # use flash_attention_2
            attn_kwargs: torch.Tensor = attention_mask if (attention_mask is not None and 0 in attention_mask) else None
...
def get_llama_decoder_layer_forward():
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        """
        基于transformers 4.39.3 LlamaDecoderLayer.forward 进行改进，改进点：
        a. 由于 input_layernorm 和 post_attention_layernorm 使用了ixformer 的融合算子，ln的输入输出形式有所改动。
