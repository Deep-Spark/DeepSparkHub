...
from ..modeling.llama import LlamaPipelineForwards, get_llama_flash_attention_forward, get_llama_decoder_layer_forward
...
class LlamaPolicy(Policy):
    ...
    def module_policy(self) -> Dict[Union[str, nn.Module], ModulePolicyDescription]:
        ...
        if self.shard_config.enable_flash_attention or self.shard_config.enable_sequence_parallelism:
            if not self.shard_config.use_colo_llamaflashatten: # 若使用Colo_LlamaFlashAtten，则使用其forward，不必做此替换
        ...
        # Colo_LlamaFlashAtten，IXFLlamaMLP 暂不支持 tp
        if self.shard_config.use_colo_llamaflashatten:
            self.append_or_create_submodule_replacement(
                description=SubModuleReplacementDescription(
                    suffix="self_attn",
                    target_module=Colo_LlamaFlashAtten,
                    kwargs=None,
                ),
                policy=policy,
        ...
        if self.shard_config.use_ixformer_mlp:
            self.append_or_create_submodule_replacement(
                description=SubModuleReplacementDescription(
                    suffix="mlp",
        ...
        self.append_or_create_submodule_replacement(
            description=[
                SubModuleReplacementDescription(
                    ...
                    target_module=Colo_FusedRMSNorm if self.shard_config.use_ixformer_fusedrmsnormres else norm_cls,
        ...
        if self.shard_config.use_ixformer_fusedrmsnormres:       
            self.append_or_create_method_replacement(
                description={
                    "forward": get_llama_decoder_layer_forward(),
                },
                policy=policy,
                target_key=LlamaDecoderLayer,
            )
