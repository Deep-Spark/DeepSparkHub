...
def _load_checkpoint_from_path(
    ...
    def _strip_null_extra_state(model_state_dict):
        """Remove null TE extra states saved by HF->mcore conversion."""
        null_extra_state_keys = [
            k for k, v in model_state_dict.items()
            if k.endswith("._extra_state") and v is None
        ]
        for key in null_extra_state_keys:
            del model_state_dict[key]
        if null_extra_state_keys:
            print_rank_0(
                f"  stripped {len(null_extra_state_keys)} null _extra_state entries from checkpoint"
            )

    def load_model_state_dict(module, state_dict, strict: bool):
        """Helper function to load state dict with fallback for missing extra states."""
        _strip_null_extra_state(state_dict)
        try:
