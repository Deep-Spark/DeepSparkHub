...
def create_pretraining_dataset(input_file, max_pred_length, shared_list, args, worker_init_fn,range_start, range_end):
    train_data = pretraining_dataset(input_file=input_file, max_pred_length=max_pred_length, seq_length_range_start=range_start, seq_length_range_end=range_end)
    train_data1 = pretraining_dataset(input_file=input_file, max_pred_length=max_pred_length, seq_length_range_start=range_end, seq_length_range_end=512)
    ...
    train_dataloader = DataLoader(train_data, sampler=train_sampler,
        ...
        pin_memory=True)
    train_dataloader1 = DataLoader(train_data1, sampler=train_sampler1,
        batch_size=args.train_batch_size, num_workers=0 if args.train_batch_size<=8 else 4, worker_init_fn=worker_init_fn,
        pin_memory=True)
    return train_dataloader, train_dataloader1, input_file
...
def create_eval_dataset(args, worker_init_fn):
    ...
    for eval_file in sorted(os.listdir(args.eval_dir)):
        ...
        if os.path.isfile(eval_file_path) and 'part' in eval_file_path:
            eval_data.extend(pretraining_dataset(eval_file_path, max_pred_length=args.max_predictions_per_seq,seq_length_range_start=0,seq_length_range_end=512))
...
class pretraining_dataset(Dataset):
    def __init__(self, input_file, max_pred_length, seq_length_range_start, seq_length_range_end):
        ...
        self.seq_lengths = np.array(f['input_mask']).sum(axis=1)
        for index in range(len(self.seq_lengths)):
            if self.seq_lengths[index]<=seq_length_range_end and self.seq_lengths[index]>seq_length_range_start:
                self.indexs_choose.append(index)
        self.inputs = [np.asarray(f[key][:]) for key in keys]
        for index in range(len(self.inputs)):
            if index < 3:
                self.inputs[index] = self.inputs[index][self.indexs_choose,:seq_length_range_end]
            elif index <5:
                self.inputs[index] = self.inputs[index][self.indexs_choose,:]
            else:
                self.inputs[index] = self.inputs[index][self.indexs_choose]
...
#                skipped_steps = torch.zeros((1,), dtype=torch.int, device=current_device)
        ...
        while global_step < args.max_steps and not end_training:
            ...
            seq_length_end=256
            seq_length_max=512
            train_data = pretraining_dataset(data_file, args.max_predictions_per_seq, seq_length_start, seq_length_end)
            train_data1 = pretraining_dataset(data_file, args.max_predictions_per_seq, seq_length_end, seq_length_max)
            train_sampler = RandomSampler(train_data)
            train_sampler1 = RandomSampler(train_data1)
            train_dataloader = DataLoader(train_data, sampler=train_sampler,
...
#                optimizer._init_everything()
            ...
            for f_id in range(f_start_id + 1, len(files)):
                ...
                if need_next_training_shard:
                    dataset_future = pool.submit(create_pretraining_dataset, data_file, args.max_predictions_per_seq, shared_file_list, args, worker_init_fn=worker_init, range_start=seq_length_start, range_end=seq_length_end)
                if use_cuda_graph and args.cuda_graph_mode=='full_iteration':
                ...
                for step, batch in enumerate(train_dataloader1):
                    ...
                    if args.distributed_lamb:
                        optimizer.set_is_accumulation_step(not update_step)
                        optimizer.set_last_step(step == len(train_dataloader1) - 1)
                ...
                train_dataloader, train_dataloader1, data_file = dataset_future.result(timeout=None)
