...
class LinearWarmupPolyDecayScheduler(LRScheduler):
    ...
    Applies a warm up period to the learning rate.
    """
    def __init__(self, args, optimizer, start_warmup_steps, warmup_steps, total_steps, end_learning_rate=0.0, degree=1.0, last_epoch=-1):
        self.args = args
    ...
    def step(self, epoch=None):
        ...
        if hasattr(self.optimizer, "_step"):
            self.last_epoch = self.optimizer._step + 1
        else:
            self.last_epoch += 1
        if torch.distributed.is_initialized() and (self.args.dist_backend == "nccl" or self.args.dist_backend == "gloo") and self.args.distributed_lamb:
            self.optimizer._lr = self.get_lr()[0]
        else:
            self.optimizer._lr = self.get_lr().item()
    ...
    def get_lr(self):
        ...
        #address RuntimeError: zero-dimensional tensor (at position 0) cannot be concatenated
        progress = (cond * (mod_step / (self.num_warmup_updates + 1e-6))) + \
                ((1.0 - cond) * (torch.min(torch.cat(((self.last_epoch - self.offset_step).view(-1) / self.total_steps, torch.ones(1, dtype=torch.float32, device=mod_step.device))))))
