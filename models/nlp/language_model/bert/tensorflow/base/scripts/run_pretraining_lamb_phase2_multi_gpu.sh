...
#  time $MPIRUN_CMD python3 $SCRIPT_DIR/run_pretraining.py \
  ...
  #time $MPIRUN_CMD python3 ./TensorFlow/nlp/bert/run_pretraining.py \
 # time horovodrun -np 8  python3 ./TensorFlow/nlp/bert/run_pretraining.py \
...
mpiexec --allow-run-as-root -np 2 echo abc 2>&1 > /dev/null
if [ $? != 0 ];then
	ROOT_FLAG=''
else
	ROOT_FLAG='--allow-run-as-root'
fi

time horovodrun -np 8 --autotune -H localhost:8 python3 ./run_pretraining.py \
      --input_files_dir=$INPUT_FILES \
      --init_checkpoint=$PHASE1_CKPT \
      --eval_files_dir=$EVAL_FILES\
      --output_dir=$RESULTS_DIR_PHASE2 \
