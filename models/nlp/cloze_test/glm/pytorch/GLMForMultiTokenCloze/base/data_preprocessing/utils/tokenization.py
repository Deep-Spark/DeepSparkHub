...
def prepare_tokenizer():
    tokenizer = BertWordPieceTokenizer(tokenizer_model_type="bert-large-uncased",
                                       cache_dir=None,
...
class CharacterLevelTokenizer(TextTokenizer):
    ...
    def __init__(self, **kwargs):
        ...
        self._tokens = [self.IdToToken(Id)
                        for Id in range(self.num_text_tokens)]
...
def get_corpus_freq(dataset, filepath, filetype='tsv'):
    ...
    print("length of freqs after trancating " +
          str(len(freqs_sorted)), flush=True)
...
class SentencePieceTokenizer(TextTokenizer):
    ...
    def Train(self, corpus, num_text_tokens):
        ...
        print("calling spm.SentencePieceTrainer.Train(%s)" %
              (train_string), flush=True)
...
class BertWordPieceTokenizer(Tokenizer):
    ...
    def __init__(self, tokenizer_model_type=None, cache_dir=None, add_block_symbols=False, add_sentinel_token=0,
        ...
        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            print('loading BertWordPieceTokenizer (',
                  tokenizer_model_type, ') from cache_dir ', cache_dir)
        do_lower_case = not (
...
class GPT2BPETokenizer(Tokenizer):
    def __init__(self, model_type_or_path, cache_dir=None, add_block_symbols=False, add_task_mask=False,
        ...
        if model_type_or_path.startswith('roberta'):
            ...
            self._command_tokens = [
                ...
                CommandToken('ENC', '[CLS]',
                             self.text_tokenizer.encoder['<s>']),
                CommandToken(
                    'MASK', '[MASK]', self.text_tokenizer.encoder['<mask>'], lstrip=True),
        ...
        if add_block_symbols:
            if add_task_mask:
                self._command_tokens.extend([
                    CommandToken('gMASK', '[gMASK]',
                                 self.num_tokens, lstrip=True),
                    CommandToken('sMASK', '[sMASK]',
                                 self.num_tokens + 1, lstrip=True)
                ])
                self.num_tokens += 2
                self.num_command_tokens += 2
            if add_decoder_mask:
        ...
        self._text_tokens = list(self._tokens)
        self._text_token_vocab = {k: v for k,
                                  v in self.text_tokenizer.encoder.items()}
    ...
    def EncodeAsTokens(self, text, process_fn=None):
        ...
        for token in re.findall(self.text_tokenizer.pat, processed_text):
            token = ''.join(
                self.text_tokenizer.bye_encoder[b] for b in token.encode('utf-8'))
            tokens.extend(
                bpe_token for bpe_token in self.text_tokenizer.bpe(token).split(' '))
