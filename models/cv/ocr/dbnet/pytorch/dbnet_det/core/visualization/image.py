...
from dbnet_det.core.evaluation.panoptic_utils import INSTANCE_OFFSET
...
def color_val_matplotlib(color):
    ...
    """
    color = dbnet_cv.color_val(color)
...
def imshow_det_bboxes(img,
    ...
    img = dbnet_cv.imread(img).astype(np.uint8)
...
def imshow_gt_det_bboxes(img,
    ...
    if not isinstance(result, dict):
        ...
        if segm_result is not None and len(labels) > 0:  # non empty
            segms = dbnet_cv.concat_list(segm_result)
