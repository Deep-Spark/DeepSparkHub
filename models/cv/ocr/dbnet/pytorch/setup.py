...
# def get_version():
#     version_file = 'dbnet_cv/version.py'
#     with open(version_file, 'r', encoding='utf-8') as f:
#         exec(compile(f.read(), version_file, 'exec'))
#     version = locals()['__version__']
#     local_version_identifier = os.environ.get('DBNET_CV_LOCAL_VERSION_IDENTIFIER', '')
...
def get_extensions():
    ...
    if EXT_TYPE == 'pytorch':
        ...
        # Since the PR (https://github.com/open-mmlab/dbnet_cv/pull/1463) uses
        ...
        if is_rocm_pytorch or torch.cuda.is_available() or os.getenv(
            ...
            define_macros += [('DBNET_CV_WITH_CUDA', None)]
            cuda_args = os.getenv('DBNET_CV_CUDA_ARGS')
            extra_compile_args['nvcc'] = [cuda_args] if cuda_args else []
            op_files = glob.glob('./dbnet_cv/ops/csrc/pytorch/*.cpp') + \
                glob.glob('./dbnet_cv/ops/csrc/pytorch/cpu/*.cpp') + \
                glob.glob('./dbnet_cv/ops/csrc/pytorch/cuda/*.cu') + \
                glob.glob('./dbnet_cv/ops/csrc/pytorch/cuda/*.cpp')
            extension = CUDAExtension
            include_dirs.append(os.path.abspath('./dbnet_cv/ops/csrc/common'))
            include_dirs.append(os.path.abspath('./dbnet_cv/ops/csrc/common/cuda'))
        # elif (hasattr(torch, 'is_mlu_available') and
        #         torch.is_mlu_available()) or \
        #         os.getenv('FORCE_MLU', '0') == '1':
        #     from torch_mlu.utils.cpp_extension import MLUExtension
        #     define_macros += [('DBNET_CV_WITH_MLU', None)]
        #     mlu_args = os.getenv('DBNET_CV_MLU_ARGS')
        #     extra_compile_args['cncc'] = [mlu_args] if mlu_args else []
        #     op_files = glob.glob('./dbnet_cv/ops/csrc/pytorch/*.cpp') + \
        #         glob.glob('./dbnet_cv/ops/csrc/pytorch/cpu/*.cpp') + \
        #         glob.glob('./dbnet_cv/ops/csrc/pytorch/mlu/*.cpp') + \
        #         glob.glob('./dbnet_cv/ops/csrc/common/mlu/*.mlu')
        #     extension = MLUExtension
        #     include_dirs.append(os.path.abspath('./dbnet_cv/ops/csrc/common'))
        #     include_dirs.append(os.path.abspath('./dbnet_cv/ops/csrc/common/mlu'))
        # else:
    ...
    # if EXT_TYPE == 'pytorch' and os.getenv('DBNET_CV_WITH_ORT', '0') != '0':
    ...
    #     include_path = os.path.abspath('./dbnet_cv/ops/csrc/onnxruntime')
    #     include_dirs.append(include_path)
    #     include_dirs.append(os.path.join(ort_path, 'include'))

    #     op_files = glob.glob('./dbnet_cv/ops/csrc/onnxruntime/cpu/*')
    #     if onnxruntime.get_device() == 'GPU' or os.getenv('FORCE_CUDA',
    #                                                       '0') == '1':
    #         define_macros += [('DBNET_CV_WITH_CUDA', None)]
    #         cuda_args = os.getenv('DBNET_CV_CUDA_ARGS')
    #         extra_compile_args['nvcc'] = [cuda_args] if cuda_args else []
    #         op_files += glob.glob('./dbnet_cv/ops/csrc/onnxruntime/gpu/*')
...
setup(
    name='dbnet_cv' if os.getenv('DBNET_CV_WITH_OPS', '0') == '0' else 'dbnet_cv-full',
    # version=get_version(),
    ...
    # url='https://github.com/open-mmlab/dbnet_cv',
    # author='DBNET_CV Contributors',
