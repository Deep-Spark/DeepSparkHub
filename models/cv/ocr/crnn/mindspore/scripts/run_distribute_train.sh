...
if [ "GPU" == $PLATFORM ]; then
  ...
  mpirun --allow-run-as-root -n $DEVICE_NUM --output-filename log_output --merge-stderr-to-stdout \
    python3 train.py --train_dataset=$DATASET_NAME --train_dataset_path=$PATH2 --device_target=GPU --run_distribute=True > log.txt 2>&1 &
  cd ..
  ...
  for ((i = 0; i < ${DEVICE_NUM}; i++)); do
    ...
    env >env.log
    python3 train.py --train_dataset_path=$PATH2 --run_distribute=True --train_dataset=$DATASET_NAME > log.txt 2>&1 &
    cd ..
  done
else
