...
run_ascend() {
  ...
  env >env.log
  python3 train.py --train_dataset=$DATASET_NAME --train_dataset_path=$1 --device_target=Ascend > log.txt 2>&1 &
  cd ..
}

run_gpu() {
  env >env.log
  python3 train.py --train_dataset=$DATASET_NAME --train_dataset_path=$1 --device_target=GPU  > log.txt 2>&1 &
  cd ..
}
