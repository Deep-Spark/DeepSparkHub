...
from ocrdet.core import BitmapMasks, PolygonMasks
from ocrdet.datasets_.builder import PIPELINES
from ocrdet.datasets_.pipelines.transforms import Resize
from PIL import Image
from shapely.geometry import Polygon as plg

import core.evaluation.utils as eval_utils
from basic_utils import check_argument
...
class RandomRotatePolyInstances:
    ...
    def rotate_img(self, img, angle, canvas_size):
        ...
        if self.pad_with_fixed_color:
            ...
            img_cut = ocrcv.imresize(img_cut, (canvas_size[1], canvas_size[0]))
            mask = cv2.warpAffine(
                mask,
                rotation_matrix, (canvas_size[1], canvas_size[0]),
                borderValue=[1, 1, 1])
...
class SquareResizePad:
    ...
    def resize_img(self, img, keep_ratio=True):
        ...
        if keep_ratio:
            ...
            t_h = t_w = self.target_size
        img = ocrcv.imresize(img, (t_w, t_h))
        return img, (t_h, t_w)
    ...
    def square_pad(self, img):
        ...
        if self.pad_with_fixed_color:
            ...
            (h_ind, w_ind) = (np.random.randint(0, h * 7 // 8),
                              np.random.randint(0, w * 7 // 8))
            img_cut = img[h_ind:(h_ind + h // 9), w_ind:(w_ind + w // 9)]
            expand_img = ocrcv.imresize(img_cut, (pad_size, pad_size))
        if h > w:
            y0, x0 = 0, (h - w) // 2
        else:
            y0, x0 = (w - h) // 2, 0
...
class RandomScaling:
    ...
    def __call__(self, results):
        ...
        image = ocrcv.imresize(image, out_size[::-1])
...
class PyramidRescale:
    ...
    def __call__(self, results):
        ...
        scale_img = ocrcv.imresize(img, (self.base_w, self.base_h))
        for _ in range(self.factor):
            scale_img = cv2.pyrDown(scale_img)
        scale_img = ocrcv.imresize(scale_img, (src_w, src_h))
