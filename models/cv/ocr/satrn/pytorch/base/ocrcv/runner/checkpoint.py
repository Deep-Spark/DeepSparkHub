...
def _get_mmcv_home():
    mmcv_home = os.path.expanduser(
        os.getenv(
            ENV_MMCV_HOME,
            os.path.join(
                os.getenv(ENV_XDG_CACHE_HOME, DEFAULT_CACHE_DIR), 'satrn')))
...
def get_external_models():
    ...
    default_json_path = osp.join(satrn.__path__[0], 'model_zoo/open_mmlab.json')
...
def get_mmcls_models():
    mmcls_json_path = osp.join(satrn.__path__[0], 'model_zoo/mmcls.json')
...
def get_deprecated_model_names():
    deprecate_json_path = osp.join(satrn.__path__[0],
...
class CheckpointLoader:
    ...
    def load_checkpoint(cls, filename, map_location=None, logger=None):
        ...
        satrn.print_log(f'Use {class_name} loader', logger)
...
def save_checkpoint(model, filename, optimizer=None, meta=None):
    ...
    #meta.update(mmcv_version=satrn.__version__, time=time.asctime())
    meta.update(mmcv_version="0.0.0.0", time=time.asctime())
    ...
    if filename.startswith('pavi://'):
        ...
        satrn.mkdir_or_exist(osp.dirname(filename))
