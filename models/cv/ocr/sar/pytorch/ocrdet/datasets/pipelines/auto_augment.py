...
class Rotate:
    ...
    Args:
        ...
        center (int | float | tuple[float]): Center point (w, h) of the
            ...
            image will be used. Same in ``ocrcv.imrotate``.
    ...
    def _rotate_img(self, results, angle, center=None, scale=1.0):
        ...
        Args:
            ...
            angle (float): Rotation angle in degrees, positive values
                mean clockwise rotation. Same in ``ocrcv.imrotate``.
            center (tuple[float], optional): Center point (w, h) of the
                rotation. Same in ``ocrcv.imrotate``.
            scale (int | float): Isotropic scale factor. Same in
                ``ocrcv.imrotate``.
        """
        for key in results.get('img_fields', ['img']):
            img = results[key].copy()
            img_rotated = ocrcv.imrotate(
...
class Translate:
    ...
    def _translate_seg(self,
        ...
        for key in results.get('seg_fields', []):
            seg = results[key].copy()
            results[key] = ocrcv.imtranslate(seg, offset, direction,
...
class ColorTransform:
    ...
    def _adjust_color_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            img = results[key]
            results[key] = ocrcv.adjust_color(img, factor).astype(img.dtype)
...
class EqualizeTransform:
    ...
    def _imequalize(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = ocrcv.imequalize(img).astype(img.dtype)
...
class BrightnessTransform:
    ...
    def _adjust_brightness_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = ocrcv.adjust_brightness(img,
...
class ContrastTransform:
    ...
    def _adjust_contrast_img(self, results, factor=1.0):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = ocrcv.adjust_contrast(img, factor).astype(img.dtype)
