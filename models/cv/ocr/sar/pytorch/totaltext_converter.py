...
from datasets_.pipelines.crop import crop_img
from basic_utils.fileio import list_to_file
...
def collect_annotations(files, nproc=1):
    ...
    if nproc > 1:
        images = ocrcv.track_parallel_progress(
            load_img_info, files, nproc=nproc)
    else:
        images = ocrcv.track_progress(load_img_info, files)
...
def generate_ann(root_path, split, image_infos):
    ...
    for image_info in image_infos:
        ...
        image = ocrcv.imread(src_img_path)
        ...
        for anno in image_info['anno_info']:
            ...
            ocrcv.imwrite(dst_img, dst_img_path)
...
def load_img_info(files):
    ...
    img = ocrcv.imread(img_file, 'unchanged')
