...
from seresnext import resnext101_32x8d
...
def main(args):
    ...
    model = None
    if args.model == "seresnext101_32x8d":
        model = resnext101_32x8d(pretrained=args.pretrained, num_classes=num_classes)
    elif args.model == "seresnext50_32x4d":
        model = resnext50_32x4d(pretrained=args.pretrained, num_classes=num_classes)
    else:
        raise RuntimeError("model {} is not support ".format(args.model))
    
    if model is None:
        raise RuntimeError("no model init... ")
    model.to(device)
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--data-path', default='../../../../../data/datasets/imagenette/', help='dataset')
    parser.add_argument('--model', default='serenext101_32x8d', help='model')
