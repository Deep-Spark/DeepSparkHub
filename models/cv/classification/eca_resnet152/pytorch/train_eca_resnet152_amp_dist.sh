...
PYTHONARG='-m torch.distributed.launch --nproc_per_node auto --use_env --master_port 10012'
# For debugging
if [ ! -z "${DEBUG}" ];then
...
python3  $PYTHONARG ${ROOT_DIR}/run_train.py \
	--model eca_resnet152 --dali --dali-cpu  --data-path $DATA_PATH \
	--opt fused_sgd --batch-size 180  \
	--lr 0.01 --amp "$@"
