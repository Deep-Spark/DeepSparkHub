...
class DetectionLoader():
    ...
    def image_preprocess(self):
        ...
        for i in range(self.num_batches):
            ...
            images_infor.append([imgs, orig_imgs, im_names, im_dim_list])
    ...
    def image_detection(self, images_infor):
        det_res = []
        for i in range(self.num_batches):
            # imgs, orig_imgs, im_names, im_dim_list = self.wait_and_get(self.image_queue)
            imgs, orig_imgs, im_names, im_dim_list = images_infor[i]
            ...
            for k in range(len(orig_imgs)):
                ...
                det_res.append([orig_imgs[k], im_names[k], boxes_k, scores[dets[:, 0] == k], ids[dets[:, 0] == k], inps, cropped_boxes])
        return det_res

    def image_postprocess(self, det_res):
        pose_inps = []
        # import pdb 
        # pdb.set_trace()
        ...
        for i in range(len(det_res)):
            with torch.no_grad():
                ...
                orig_img, im_name, boxes, scores, ids, inps, cropped_boxes = det_res[i]
                ...
                pose_inps.append([inps, orig_img, im_name, boxes, scores, ids, cropped_boxes])
        return pose_inps
    ...
    def det_res(self):
        images_infor = self.image_preprocess()
        det_res = self.image_detection(images_infor)
        pose_inps = self.image_postprocess(det_res)
        return pose_inps
