...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        "--lr-steps-official",
        help="Use official lr-steps refer to Paper",
        action="store_true",
    )
...
def main(args):
    ...
    dataset, num_classes = get_dataset(args.dataset, "train", args.data_path)
    dataset_test, _ = get_dataset(args.dataset, "val", args.data_path)
    train_samples = len(dataset)
    val_samples = len(dataset_test)
    print("train datasets samples: {}, val datasets samples: {}\nCreating data loaders".format(train_samples, val_samples))
    if args.lr_steps_official:
        # refer to Paper[3.1 PASCAL VOC2007]
        args.lr_steps = [int(400000*32/train_samples), int(600000*32/train_samples)]
    ...
    print("""Training parameters: \nbatch_size: {}, global_batch_size: {}, base_lr: {}, lr_steps: {}, 
        momentum: {}, weight_decay: {}""".format(args.batch_size, args.batch_size * utils.get_world_size(),
        args.lr, args.lr_steps, args.momentum, args.weight_decay))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(model, optimizer, data_loader, device, epoch, args.print_freq, args.amp)
        lr_scheduler.step()

        # evaluate after every epoch
        coco_evaluator, res = evaluate(model, data_loader_test, device=device)
        print("epoch: {}".format(epoch), res)
        iou_type, eval_map = res[0]
        if args.output_dir:
            checkpoint = {
            ...
            utils.save_on_master(
                checkpoint,
                os.path.join(args.output_dir, 'epoch{}_{}.pth'.format(epoch, eval_map)))
