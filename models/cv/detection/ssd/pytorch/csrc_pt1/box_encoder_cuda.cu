...
std::vector<at::Tensor> box_encoder(const int N_img,
  ...
  at::Tensor labels_out = torch::empty({N_img, M}, labels_input.options());
  THCudaCheck(cudaGetLastError());

  // copy default boxes to outputs
  at::Tensor bbox_out = dbox.repeat({N_img, 1}).view({N_img, M, 4});
  THCudaCheck(cudaGetLastError());

  // need to allocate some workspace
  // at::Tensor workspace = at::CUDA(at::kByte).zeros({8 * M * N_img});
  at::Tensor workspace = torch::zeros({8 * M * N_img}, at::CUDA(at::kByte));
  THCudaCheck(cudaGetLastError());
...
at::Tensor calc_ious(const int N_img,
  ...
  at::Tensor ious = torch::empty({N, M}, boxes1.options());
