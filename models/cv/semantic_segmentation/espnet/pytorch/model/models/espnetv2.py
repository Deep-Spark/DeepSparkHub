...
class ESPNetV2(SegBaseModel):
    ...
    def __init__(self, cfg, **kwargs):
        super(ESPNetV2, self).__init__(cfg)
    ...
    def forward(self, x):
        ...
        up_l4_to_l3 = F.interpolate(
            out_l4_proj, size=out_l3.size()[2:], mode='bilinear', align_corners=True)
        ...
        out_up_l3 = F.interpolate(
            proj_merge_l3, size=out_l2.size()[2:], mode='bilinear', align_corners=True)
        merge_l2 = self.project_l2(torch.cat([out_l2, out_up_l3], 1))
        out_up_l2 = F.interpolate(
            merge_l2, size=out_l1.size()[2:], mode='bilinear', align_corners=True)
