...
class DFANet(SegBaseModel):
    def __init__(self, cfg, **kwargs):
        super(DFANet, self).__init__(cfg)
    ...
    def forward(self, x):
        ...
        stage1_out = F.interpolate(
            stage1_fca, size=stage1_enc2.size()[2:], mode='bilinear', align_corners=True)
        ...
        stage2_out = F.interpolate(
            stage2_fca, size=stage2_enc2.size()[2:], mode='bilinear', align_corners=True)
        ...
        stage2_enc2_docoder = F.interpolate(self.enc2_2_reduce(stage2_enc2),
                                            size=stage1_enc2_decoder.size()[2:],
        ...
        size = fusion.size()[2:]
        stage1_fca_decoder = F.interpolate(self.fca_1_reduce(stage1_fca), size,
                                           mode='bilinear', align_corners=True)
        stage2_fca_decoder = F.interpolate(self.fca_2_reduce(stage2_fca), size,
                                           mode='bilinear', align_corners=True)
        stage3_fca_decoder = F.interpolate(self.fca_3_reduce(stage3_fca), size,
                                           mode='bilinear', align_corners=True)
