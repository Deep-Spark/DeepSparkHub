...
def evaluate(flags, model, loader, loss_fn, score_fn, device, epoch=0, is_distributed=False):
    ...
    if flags.load_ckpt_path:
        ...
        checkpoint = torch.load(flags.load_ckpt_path, map_location='cpu')
        epoch = checkpoint['epoch']
        
        model_without_ddp = model
        print('load success')
        print('is_distributed:',is_distributed,[flags.local_rank])
        if is_distributed:
    ...
    scores, eval_loss = scores.cpu().numpy().tolist(), float(eval_loss.cpu().numpy())
